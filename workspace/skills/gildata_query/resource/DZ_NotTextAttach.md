...
1.内容说明：本表为DZ_NotTextAnnouncement的附表，用于存放DZ_NotTextAnnouncement表对应公告非文本的其他格式类型文件。文件格式包含：json、html；
2.数据范围：1988年至今；
3.信息来源：恒生聚源。
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `RID` | RID | number(19) | ✗ | 100.0% | RID：与DZ_NotTextAnnouncement的ID关联，获取公告的相关信息，包含内部编码、媒体出处、信息标题等... |
...
RID：与DZ_NotTextAnnouncement的ID关联，获取公告的相关信息，包含内部编码、媒体出处、信息标题等。
