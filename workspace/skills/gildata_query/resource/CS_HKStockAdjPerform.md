...
| 属性 | 值 |
|------|-----|
| 表名 | `CS_HKStockAdjPerform` |
| MySQL表名 | `cs_hkstockadjperform` |
| 中文名 | 港股复权行情表 |
...
| 4 | `ExDiviDate` | 除权除息日 | date | ✓ | 81.63% |  |
| 5 | `AdjPrevClose` | 复权昨收盘(元) | number(19,4) | ✓ | 99.79% |  |
| 6 | `AdjOpenPrice` | 复权开盘价(元) | number(19,4) | ✓ | 99.79% |  |
| 7 | `AdjHighPrice` | 复权最高价(元) | number(19,4) | ✓ | 99.79% |  |
| 8 | `AdjLowPrice` | 复权最低价(元) | number(19,4) | ✓ | 99.79% |  |
| 9 | `AdjClosePrice` | 复权收盘价(元) | number(19,4) | ✓ | 99.79% |  |
| 10 | `AdjustingMethod` | 复权方式 | number(10) | ✗ | 100.0% | 复权方式(AdjustingMethod)与(CT_SystemConst)表中的DM字段关联，令LB=2544，得到复... |
| 11 | `AdjustingStandard` | 复权标准 | number(10) | ✗ | 100.0% | 复权标准(AdjustingStandard)与(CT_SystemConst)表中的DM字段关联，令LB=2545，得... |
| 12 | `AdjustingFactor` | 复权因子 | number(19,4) | ✓ | 100.0% |  |
| 13 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 100.0% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB=1068，得到货币单位... |
