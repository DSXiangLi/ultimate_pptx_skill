...
| 属性 | 值 |
|------|-----|
| 表名 | `DZ_AfterDailyQuote` |
| MySQL表名 | `dz_afterdailyquote` |
| 中文名 | A股盘后日行情 |
...
1.内容说明：收录股票每个交易日盘后以固定价格交易的行情数据，包括收盘价、买入申报数量、卖出申报数量、盘后成交量、盘后成交金额等行情指标。
2.数据范围：证券上市起-至今
3.信息来源：上交所，深交所每日行情文件
...
| 7 | `AfterBuyVolume` | 盘后买入申报数量(股/份) | number(16,0) | ✓ | 28.55% |  |
| 8 | `AfterSellVolume` | 盘后卖出申报数量(股/份) | number(16,0) | ✓ | 28.55% |  |
