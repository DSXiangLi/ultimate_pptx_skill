...
| 属性 | 值 |
|------|-----|
| 表名 | `Bond_IssuerCreditLine` |
| MySQL表名 | `bond_issuercreditline` |
| 中文名 | 债券发行人授信情况 |
...
| 3 | `CompanyCode` | 债券发行人公司代码 | number(10) | ✓ | 100.0% | 债券发行人公司代码（CompanyCode）：与“证券主表（SecuMain）”中的“公司代码（CompanyCode）... |
| 4 | `InfoPublDate` | 公告日期 | date | ✓ | 100.0% |  |
| 5 | `InfoSource` | 信息来源 | varchar2(200) | ✓ | 100.0% |  |
| 6 | `EndDate` | 截止日期 | date | ✓ | 100.0% |  |
| 7 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 100.0% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1548 and ... |
| 8 | `AmountOfCreditLine` | 授信额度(亿元) | number(19,4) | ✓ | 98.12% |  |
| 9 | `AmountOfDrawdown` | 已使用额度(亿元) | number(19,4) | ✓ | 96.97% |  |
| 10 | `OutstandingAmount` | 未使用额度(亿元) | number(19,4) | ✓ | 98.84% |  |
| 11 | `CreditLineRemark` | 授信说明 | clob | ✓ | 0.34% |  |
...
### CompanyCode (债券发行人公司代码)

债券发行人公司代码（CompanyCode）：与“证券主表（SecuMain）”中的“公司代码（CompanyCode）”关联，得到上市公司的交易代码、简称等。
