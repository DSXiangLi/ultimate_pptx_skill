...
| 5 | `ParentCompany` | 所属公司 | number(10) | ✓ | 100.0% | 所属公司（ParentCompany）：与“机构基本资料（LC_InstiArchive）”中的“企业编号（Compan... |
| 6 | `EndDate` | 截止日期 | date | ✗ | 100.0% |  |
| 7 | `IfMerged` | 合并标志 | number(10) | ✗ | 100.0% | 合并标志（IfMerged），该字段固定以下常量：1-合并；2-母公司 |
| 8 | `IfAdjusted` | 调整标志 | number(10) | ✗ | 100.0% | 调整标志（IfAdjusted），该字段固定以下常量：1-是；2-否 |
| 9 | `DataType` | 统计类别 | number(10) | ✗ | 100.0% | 统计类别(DataType)与(CT_SystemConst)表中的DM字段关联，令LB = 1074 AND DM I... |
| 10 | `FinancialItemName` | 财务科目名称 | varchar2(100) | ✓ | 100.0% |  |
| 11 | `FinancialItemCode` | 财务科目代码 | number(10) | ✗ | 100.0% | 财务科目代码（FinancialItemCode)，该字段固定以下常量：11-营业收入；23-净利润；3090-净资产；... |
| 12 | `Currency` | 计价货币 | number(10) | ✓ | 100.0% | 计价货币(Currency)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到计价货币的具... |
| 13 | `FinancialItemUint` | 财务科目单位 | number(10) | ✓ | 100.0% | 财务科目单位(FinancialItemUint)与(CT_SystemConst)表中的DM字段关联，令LB = 12... |
| 14 | `FinancialItemData` | 财务科目数据 | number(19,6) | ✓ | 100.0% |  |
| 15 | `Notes` | 备注 | varchar2(500) | ✓ | 0.0% |  |
