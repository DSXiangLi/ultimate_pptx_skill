...
内容说明：收录MSCI指数当日成份开盘、收盘权重及股本、市值等信息。
...
| 3 | `IndexCode` | 指数内部编码 | number(10) | ✗ | 100.0% | 指数内部编码(IndexCode)：与“证券主表(SecuMain)”中的“证券内部编码(InnerCode)”关联，得... |
| 4 | `MSCode` | MS成份券内码 | number(10) | ✗ | 100.0% |  MS成份券内码(MSCode)：与“MSCI指数成份当日基本信息(Index_MSCISecuInfo)”中的“MS成... |
| 5 | `InnerCode` | 证券内部编码 | number(10) | ✓ | 49.6% | 证券内部编码(InnerCode)：关联不同主表，查询证券代码、证券简称等基本信息。当0<SecuInnerCode<=... |
| 6 | `RIC` | 路孚特代码 | varchar2(100) | ✓ | 98.21% |  |
| 7 | `SecuMarket` | 证券市场 | number(10) | ✓ | 100.0% | 证券市场(SecuMarket)与(CT_SystemConst)表中的DM字段关联，令LB = 201，得到证券市场的... |
| 8 | `OpenWeightedRatio` | 开盘权重(%) | number(22,13) | ✓ | 100.0% |  |
| 9 | `WeightedRatio` | 收盘权重(%) | number(22,13) | ✓ | 99.12% |  |
| 10 | `FullMCAdjFactor` | 总市值调整因子 | number(22,9) | ✓ | 99.48% |  |
| 11 | `InitialMV` | 初始市值(美元) | number(28,6) | ✓ | 99.48% |  |
| 12 | `CloseMV` | 收盘市值(美元) | number(28,6) | ✓ | 99.23% |  |
| 13 | `AdjustedMV` | 调整市值(美元) | number(28,6) | ✓ | 99.48% |  |
| 14 | `PrevClosePrice` | 昨收盘 | number(22,5) | ✓ | 52.63% |  |
| 15 | `TotalShares` | 总股本(股) | number(22,4) | ✓ | 52.63% |  |
| 16 | `AdjustedShares` | 计算用股本(股) | number(22,4) | ✓ | 52.63% |  |
| 17 | `InitialTMV` | 初始总市值(美元) | number(28,6) | ✓ | 52.63% |  |
| 18 | `CloseTMV` | 收盘总市值(美元) | number(28,6) | ✓ | 52.63% |  |
| 19 | `CurrencyCode` | 币种 | number(10) | ✓ | 52.62% | 币种(CurrencyCode)与(CT_SystemConst)表中的DM字段关联，令LB=1548，得到币种的具体描... |
| 20 | `ExchangeRate` | 汇率 | number(22,15) | ✓ | 52.63% |  |
| 21 | `AdjustType` | 调整类型 | number(10) | ✓ | 0.0% | 调整类型(AdjustType)与(CT_SystemConst)表中的DM字段关联，令LB=2554，得到调整类型的具... |
| 22 | `IfAdjusted` | 是否调整价格 | number(10) | ✓ | 52.62% | 是否调整价格(IfAdjusted)，该字段固定常量以下常量：1-是；0-否 |
| 23 | `PriceAdjFactor` | 价格调整因子 | number(19,6) | ✓ | 0.0% |  |
| 24 | `IfRebalancing` | 是否调样中 | number(10) | ✓ | 52.63% | 是否调样中(IfRebalancing)，该字段固定常量以下常量：1-是；0-否 |
...
调整类型(AdjustType)与(CT_SystemConst)表中的DM字段关联，令LB=2554，得到调整类型的具体描述：1-纳入，2-剔除，3-股本变动，4-调整因子变动，5-调整因子变动和股本变动。

### IfAdjusted (是否调整价格)

是否调整价格(IfAdjusted)，该字段固定常量以下常量：1-是；0-否

### IfRebalancing (是否调样中)

是否调样中(IfRebalancing)，该字段固定常量以下常量：1-是；0-否
