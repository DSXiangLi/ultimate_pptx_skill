...
def query_fund_performance(conn, inner_code: int):
    """查询基金业绩表现"""
    with conn.cursor() as cur:
        sql = """
        SELECT 
        ...
        for row in rows:
            data.append({
                ...
                "日涨跌幅(%)": round(row[2], 2) if row[2] else None
            })
...
def main():
    ...
    try:
        ...
        # 查询业绩
        df = query_fund_performance(conn, fund['InnerCode'])
