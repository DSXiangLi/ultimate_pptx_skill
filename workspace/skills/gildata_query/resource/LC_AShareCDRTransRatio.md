...
| 属性 | 值 |
|------|-----|
| 表名 | `LC_AShareCDRTransRatio` |
| MySQL表名 | `lc_asharecdrtransratio` |
| 中文名 | CDR与基础证券转换 |
...
1.内容说明：本表记录A股市场上市的中国存托凭证与其基础证券的转换比例、存托凭证份数、面值等信息。
2.数据范围：CDR上市至今
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `InnerCode` | 存托凭证内部编码 | number(10) | ✗ | 0.0% | 存托凭证内部编码(InnerCode)：与“证券主表（SecuMain）”中的“证券内部编码（InnerCode）”关联... |
| 3 | `DRSharesNum` | 存托凭证份数 | number(18,2) | ✓ | 0.0% | 该存托凭证份数的有效时间仅为当前生效日期对应的存托凭证份数 |
| 4 | `BasicSecuSharesNum` | 基础证券股数 | number(18,2) | ✓ | 0.0% | 该基础证券股数的有效时间仅为当前生效日期对应的基础证券股数 |
| 5 | `BasicSecuType` | 基础证券种类 | varchar2(50) | ✓ | 0.0% |  |
| 6 | `TransRatio` | 转换比例 | number(18,6) | ✓ | 0.0% |  |
| 7 | `EffectiveDate` | 生效日期 | date | ✗ | 0.0% |  |
| 8 | `ExpiryDate` | 失效日期 | date | ✓ | 0.0% |  |
| 9 | `IfEffected` | 是否有效 | number(10) | ✗ | 0.0% | 是否有效(IfEffected)与(CT_SystemConst)表中的DM字段关联，令LB = 999 and DM ... |
| 10 | `DRParValue` | 存托凭证面值 | number(19,4) | ✓ | 0.0% |  |
| 11 | `DRParValueUnit` | 存托凭证面值单位 | number(10) | ✓ | 0.0% | 存托凭证面值单位(DRParValueUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 106... |
| 12 | `BasicSecuParValue` | 基础股票面值 | number(25,10) | ✓ | 0.0% |  |
| 13 | `BasicSecuParValueUnit` | 基础股票面值单位 | number(10) | ✓ | 0.0% | 基础股票面值单位(BasicSecuParValueUnit)与(CT_SystemConst)表中的DM字段关联，令L... |
...
存托凭证内部编码(InnerCode)：与“证券主表（SecuMain）”中的“证券内部编码（InnerCode）”关联，取““证券类别(SecuCategory)”=41-中国存托凭证，得到存托凭证的交易代码、简称等。

### DRSharesNum (存托凭证份数)

该存托凭证份数的有效时间仅为当前生效日期对应的存托凭证份数

### BasicSecuSharesNum (基础证券股数)

该基础证券股数的有效时间仅为当前生效日期对应的基础证券股数
