...
| 属性 | 值 |
|------|-----|
| 表名 | `DZ_InterimBulletin` |
| MySQL表名 | `dz_interimbulletin` |
| 中文名 | 公司临时公告 |
...
1.内容说明：公司历年披露的除发行上市公告书、定期报告等以外的其他各类公告，其他机构如交易所、证监会、拍卖行等发布的相关公告。
2.数据范围：证券上市-至今
...
| 4 | `Detail` | 详细内容 | clob | ✓ | 2.2% |  |
| 5 | `BulletinType` | 公告类型 | number(10) | ✓ | 100.0% | 公告类型(BulletinType)与(CT_SystemConst)表中的DM字段关联，令LB = 1109，得到公告... |
| 6 | `Market` | 对应市场 | number(10) | ✓ | 93.28% | 对应市场(Market)与(CT_SystemConst)表中的DM字段关联，令LB = 201，得到对应市场的具体描述... |
| 7 | `Category` | 对应类别 | number(10) | ✓ | 99.93% | 对应类别(Category)与(CT_SystemConst)表中的DM字段关联，令LB = 1177，得到对应类别的具... |
| 8 | `Media` | 媒体出处 | number(10) | ✓ | 98.35% |  |
| 9 | `InvolvedStock` | 相关股票 | number(10) | ✓ |  |  |
| 10 | `Keywords` | 相关关键词 | number(10) | ✓ |  |  |
| 11 | `RecordDate` | 记录录入时间 | date | ✓ | 100.0% |  |
