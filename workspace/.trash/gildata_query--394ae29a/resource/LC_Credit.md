...
1.收录上市公司公告中披露的公司借贷等重大事项描述，包括时间内容、时间主体、交易对象名称、借贷金额、还款金额、借贷利率、借贷期限等指标。
2.数据范围：2001年-至今
3.信息来源：上市公司公告
...
| 3 | `InitialInfoPublDate` | 首次信息发布日期 | date | ✓ | 98.44% |  |
| 4 | `InfoPublDate` | 信息发布日期 | date | ✗ | 100.0% |  |
| 5 | `InfoSource` | 信息来源 | varchar2(50) | ✓ | 98.44% |  |
| 6 | `AnnouncementType` | 公告类型 | number(10) | ✓ | 100.0% | 公告类型(AnnouncementType)与(CT_SystemConst)表中的DM字段关联，令LB = 1109，... |
| 7 | `DisclosureMethod` | 披露方式 | number(10) | ✓ | 98.44% | 披露方式(DisclosureMethod)与(CT_SystemConst)表中的DM字段关联，令LB = 1221，... |
| 8 | `EventContent` | 事件内容 | clob | ✓ | 1.01% |  |
| 9 | `ActionDesc` | 行为描述 | varchar2(200) | ✓ | 98.44% |  |
| 10 | `NewestAdvance` | 最新进展状态描述 | varchar2(200) | ✓ | 43.42% |  |
| 11 | `EventSubject` | 事件主体 | number(10) | ✓ | 99.98% | 事件主体(EventSubject)与(CT_SystemConst)表中的DM字段关联，令LB = 1246，得到事件... |
| 12 | `EventProcedure` | 事件进程 | number(10) | ✓ | 98.44% | 事件进程(EventProcedure)与(CT_SystemConst)表中的DM字段关联，令LB = 1059，得到... |
| 13 | `ActionWays` | 行为方式 | number(10) | ✓ | 98.44% | 行为方式(ActionWays)与(CT_SystemConst)表中的DM字段关联，令LB = 1063，得到行为方式... |
| 14 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 98.42% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币... |
| 15 | `SubjectName` | 事件主体名称 | varchar2(100) | ✓ | 98.44% |  |
| 16 | `SubjectCode` | 事件主体企业编号 | number(10) | ✓ | 52.12% | 事件主体企业编号(SubjectCode)：与“机构基本资料表(LC_InstiArchive)”中的“公司代码(Com... |
| 17 | `SubjectAssociation` | 与上市公司关联关系 | number(10) | ✓ | 98.44% | 与上市公司关联关系(SubjectAssociation)与(CT_SystemConst)表中的DM字段关联，令LB ... |
| 18 | `ObjectName` | 交易对象名称 | varchar2(100) | ✓ | 98.44% |  |
| 19 | `ObjectCode` | 交易对象企业编号 | number(10) | ✓ | 27.44% | 交易对象企业编号(ObjectCode)：与“机构基本资料表(LC_InstiArchive)”中的“公司代码(Comp... |
| 20 | `ObjectAssociation` | 与上市公司关联关系 | number(10) | ✓ | 98.44% | 与上市公司关联关系(ObjectAssociation)与(CT_SystemConst)表中的DM字段关联，令LB =... |
| 21 | `AgreementDate` | 协议签署日期 | date | ✓ | 1.22% |  |
| 22 | `IfEnded` | 是否终止 | number(10) | ✓ | 1.35% | 是否终止(IfEnded)，该字段固定以下常量：0-否；1-是 |
| 23 | `Note` | 备注 | varchar2(300) | ✓ | 0.02% |  |
| 24 | `LoanCondition` | 借贷条件 | number(10) | ✓ | 63.48% | 借贷条件(LoanCondition)与(CT_SystemConst)表中的DM字段关联，令LB = 1057，得到借... |
| 25 | `Borrower` | 借入方 | varchar2(50) | ✓ | 98.44% |  |
| 26 | `BorrowerAssociation` | 与上市公司关联关系 | number(10) | ✓ | 98.44% | 与上市公司关联关系(BorrowerAssociation)与(CT_SystemConst)表中的DM字段关联，令LB... |
| 27 | `MortgageAsset` | 借入方抵押置押资产 | varchar2(100) | ✓ | 4.78% |  |
| 28 | `Lender` | 借出方 | varchar2(50) | ✓ | 98.44% |  |
| 29 | `LenderAssociation` | 与上市公司关联关系 | number(10) | ✓ | 98.44% | 与上市公司关联关系(LenderAssociation)与(CT_SystemConst)表中的DM字段关联，令LB =... |
| 30 | `Guarantor` | 担保方 | varchar2(50) | ✓ | 54.64% |  |
| 31 | `GuarantorAssociation` | 与上市公司关联关系 | number(10) | ✓ | 54.64% | 与上市公司关联关系(GuarantorAssociation)与(CT_SystemConst)表中的DM字段关联，令L... |
| 32 | `GuarantorMortgageAsset` | 担保方抵押置押资产 | varchar2(100) | ✓ | 1.84% |  |
| 33 | `FirstLoanSum` | 首次借贷金额(元) | number(19,4) | ✓ | 98.43% |  |
| 34 | `LatestLoanSum` | 最新借贷金额(元) | number(19,4) | ✓ | 99.99% |  |
| 35 | `LatestRepaymentSum` | 最近还款金额(元) | number(19,4) | ✓ | 0.39% |  |
| 36 | `AccumulatedRepaymentSum` | 累计还款金额(元) | number(19,4) | ✓ | 0.39% |  |
| 37 | `YearRateStat` | 借贷年利率说明 | varchar2(50) | ✓ | 5.62% |  |
| 38 | `YearRate` | 借贷年利率 | number(9,6) | ✓ | 2.56% |  |
| 39 | `LoanTerm` | 借贷期限(月) | number(18,4) | ✓ | 42.22% |  |
| 40 | `RenewalTerm` | 其中：续借期限(月) | number(18,4) | ✓ | 0.68% |  |
| 41 | `LoanBeginDate` | 借贷起始日 | date | ✓ | 4.23% |  |
| 42 | `LoanEndDate` | 借贷截止日 | date | ✓ | 5.07% |  |
| 43 | `LoanGuaranteeTerm` | 担保期限(月) | number(18,4) | ✓ | 19.67% |  |
| 44 | `LoanRenewalGuaranteeTerm` | 其中：续保期限(月) | number(18,4) | ✓ | 0.16% |  |
| 45 | `LoanGuaranteeBeginDate` | 担保起始日 | date | ✓ | 2.08% |  |
| 46 | `LoanGuaranteeEndDate` | 担保截止日 | date | ✓ | 2.51% |  |
| 47 | `LatestRepaymentDate` | 最近还款日期 | date | ✓ | 0.09% |  |
| 48 | `XGRQ` | 修改日期 | date | ✗ |  |  |
...
事件主体企业编号(SubjectCode)：与“机构基本资料表(LC_InstiArchive)”中的“公司代码(CompanyCode)”关联，得到企业的信息。
...
交易对象企业编号(ObjectCode)：与“机构基本资料表(LC_InstiArchive)”中的“公司代码(CompanyCode)”关联，得到企业的信息。
