...
def build_pptx(ir, pptx_path, report_path):
    ...
    slide_ids = "\n".join('<p:sldId id="%s" r:id="rId%s"/>' % (255+i, i) for i in range(1, len(slide_xmls) + 1))
    slide_rels = "\n".join('<Relationship Id="rId%s" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slide" Target="slides/slide%s.xml"/>' % (i, i) for i in range(1, len(slide_xmls) + 1))
    files = {
...
<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" xmlns:vt="http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes"><Application>ultimate-pptx-builder</Application></Properties>''',
