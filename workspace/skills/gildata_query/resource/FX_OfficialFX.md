...
1.收录香港金管局披露的官方外汇汇率数据、国际货币基金组织IMF和中国外汇交易中心CFETS公布的SDR（特别提款权）对CNY（人民币）的汇率
2.数据范围：1981年1月-至今
3.数据源：香港金融管理局、外汇交易中心
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `InfoPublDate` | 信息发布日期 | date | ✓ | 99.48% |  |
| 3 | `EndDate` | 截止日期 | date | ✗ | 100.0% |  |
| 4 | `InfoSource` | 数据来源 | varchar2(50) | ✓ | 100.0% |  |
| 5 | `ReportPeriod` | 数据统计期间 | number(10) | ✗ | 100.0% | 数据统计期间(ReportPeriod):与“系统常量表(CT_SystemConst)”中的“代码(DM)”关联，令“... |
| 6 | `DomeCurrency` | 本币种类 | number(10) | ✗ | 100.0% | 本币种类(DomeCurrency)与(CT_SystemConst)表中的DM字段关联，令LB = 1068 AND ... |
| 7 | `ForeCurrency` | 外币种类 | number(10) | ✗ | 100.0% | 外币种类(ForeCurrency)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到外币... |
| 8 | `MidExRate` | 汇率中间价 | number(12,6) | ✓ | 100.0% |  |
