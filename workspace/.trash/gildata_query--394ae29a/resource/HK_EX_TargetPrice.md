...
| 属性 | 值 |
|------|-----|
| 表名 | `HK_EX_TargetPrice` |
| MySQL表名 | `hk_ex_targetprice` |
| 中文名 | 港股目标价 |
...
1.内容说明：本表统计各研究机构对港股上市公司流通股票目标价的预测情况
2.数据范围：2020年至今数据
3.信息来源：FactSet
...
| 5 | `CurrencyCode` | 预测数据币种 | number(10) | ✓ | 100.0% | 预测数据币种(CurrencyCode)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到... |
| 6 | `TargetPriceAvg` | 平均目标价(元) | number(19,8) | ✓ | 100.0% |  |
| 7 | `TargetPriceMedian` | 中位数目标价(元) | number(19,8) | ✓ | 100.0% |  |
| 8 | `TargetPriceNum` | 目标价机构数 | number(10) | ✓ | 100.0% |  |
| 9 | `TargetPriceMin` | 最低目标价(元) | number(19,8) | ✓ | 100.0% |  |
| 10 | `TargetPriceMax` | 最高目标价(元) | number(19,8) | ✓ | 100.0% |  |
| 11 | `TargetPriceSD` | 目标价标准差 | number(19,8) | ✓ | 80.19% |  |
| 12 | `TargetPriceUpNum` | 目标价向上修正数量 | number(10) | ✓ | 100.0% |  |
| 13 | `TargetPriceDownNum` | 目标价向下修正数量 | number(10) | ✓ | 100.0% |  |
