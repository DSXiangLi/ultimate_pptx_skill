...
| 属性 | 值 |
|------|-----|
| 表名 | `Index_MSCISecuInfo` |
| MySQL表名 | `index_mscisecuinfo` |
| 中文名 | MSCI指数成份当日基本信息 |
...
内容说明：收录MSCI指数成份当日的基本信息，包括：代码信息、证券信息、行业分类等
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `IndexCode` | 指数内码 | number(10) | ✗ | 100.0% | 指数内部代码（IndexCode）：与“证券主表（SecuMain）”中的“证券内部编码（InnerCode）”关联，得... |
| 3 | `TradingDay` | 交易日 | date | ✗ | 100.0% |  |
| 4 | `MSCode` | MS成份券内码 | number(10) | ✗ | 100.0% |  |
| 5 | `SecuMarket` | 证券市场 | number(10) | ✓ | 99.98% | 证券市场(SecuMarket)与(CT_SystemConst)表中的DM字段关联，令LB=201，得到证券市场的具体... |
| 6 | `InnerCode` | 证券内部编码 | number(10) | ✓ | 48.35% | 证券内部编码(InnerCode)：关联不同主表，查询证券代码、证券简称等基本信息。当0<SecuInnerCode<=... |
| 7 | `MSTSCode` | MS成份序列代码 | varchar2(100) | ✓ | 100.0% |  |
| 8 | `RIC` | 路孚特代码 | varchar2(100) | ✓ | 99.98% |  |
| 9 | `MIC` | 市场标识码 | varchar2(50) | ✓ | 99.97% |  |
| 10 | `ISIN` | ISIN | varchar2(50) | ✓ | 99.87% |  |
| 11 | `TICKER` | 彭博代码 | varchar2(50) | ✓ | 99.76% |  |
| 12 | `BARRAID` | BARRAID | varchar2(50) | ✓ | 100.0% |  |
| 13 | `CUSIP` | CUSIP | varchar2(50) | ✓ | 0.0% |  |
| 14 | `SEDOL` | SEDOL | varchar2(50) | ✓ | 0.0% |  |
| 15 | `ChiSecuCode` | 中国股票代码 | varchar2(50) | ✓ | 10.3% |  |
| 16 | `RIC_ND` | 次日路孚特代码 | varchar2(50) | ✓ | 99.97% |  |
| 17 | `MIC_ND` | 次日市场标识码 | varchar2(50) | ✓ | 99.97% |  |
| 18 | `ISIN_ND` | 次日ISIN | varchar2(50) | ✓ | 99.86% |  |
| 19 | `TICKER_ND` | 次日彭博代码 | varchar2(50) | ✓ | 99.76% |  |
| 20 | `CUSIP_ND` | 次日CUSIP | varchar2(50) | ✓ | 0.0% |  |
| 21 | `SEDOL_ND` | 次日SEDOL | varchar2(50) | ✓ | 0.0% |  |
| 22 | `ChiSecuCode_ND` | 次日中国股票代码 | varchar2(50) | ✓ | 10.3% |  |
| 23 | `EngName` | 英文名称 | varchar2(100) | ✓ | 100.0% |  |
| 24 | `ShareCategoryName` | 股份类型名称 | varchar2(50) | ✓ | 99.99% |  |
| 25 | `ShareCategory` | 股份类型 | number(10) | ✓ | 0.0% | 股份类型(ShareCategory)与(CT_SystemConst)表中的DM字段关联，令LB=2571，得到股份类... |
| 26 | `ShareTypeName` | 股份类别名称 | varchar2(50) | ✓ | 42.0% |  |
| 27 | `AlternateListingName` | 另类交易名称 | varchar2(50) | ✓ | 18.4% |  |
| 28 | `AlternateListing` | 另类交易 | number(10) | ✓ | 7.35% | 另类交易(AlternateListing)与(CT_SystemConst)表中的DM字段关联，令LB=2572，得到... |
| 29 | `RegionCode` | 国家及地区代码 | number(10) | ✓ | 100.0% | 国家及地区代码(RegionCode)：与“国家城市代码表(LC_AreaCode)”中的“地区内部编码(AreaInn... |
| 30 | `RegionCode_ND` | 次日国家及地区代码 | number(10) | ✓ | 99.99% | 次日国家及地区代码(RegionCode_ND)：与“国家城市代码表(LC_AreaCode)”中的“地区内部编码(Ar... |
| 31 | `FirstIndustryCode` | 一级行业代码 | varchar2(50) | ✓ | 100.0% |  |
| 32 | `SecondIndustryCode` | 二级行业代码 | varchar2(50) | ✓ | 100.0% |  |
| 33 | `ThirdIndustryCode` | 三级行业代码 | varchar2(50) | ✓ | 100.0% |  |
| 34 | `FourthIndustryCode` | 四级行业代码 | varchar2(50) | ✓ | 100.0% |  |
| 35 | `FirstIndustryCode_ND` | 次日一级行业代码 | varchar2(50) | ✓ | 99.99% |  |
| 36 | `SecondIndustryCode_ND` | 次日二级行业代码 | varchar2(50) | ✓ | 99.99% |  |
| 37 | `ThirdIndustryCode_ND` | 次日三级行业代码 | varchar2(50) | ✓ | 99.99% |  |
| 38 | `FourthIndustryCode_ND` | 次日四级行业代码 | varchar2(50) | ✓ | 99.99% |  |
| 39 | `IfPublish` | 是否出版 | number(10) | ✓ | 100.0% | 是否出版(IfPublish)，该字段固定常量以下常量：1-是；0-否 |
| 40 | `UpdateTime` | 修改时间 | date | ✗ |  |  |
...
股份类型(ShareCategory)与(CT_SystemConst)表中的DM字段关联，令LB=2571，得到股份类型的具体描述：1-不记名股票(波兰葡萄牙)，2-CHESS存托利息(澳大利亚)，3-普通股，4-分红证(瑞士)，5-不记名股票(瑞士)，6-记名股票(德国瑞士)，7-普通股(巴西)，8-普通股(意大利)，9-参股证券，10-优先股(巴西)，11-优先股，12-优先股(黎巴嫩)，13-优先股(意大利)，14-记名股票(波兰葡萄牙)，15-储蓄股票(意大利)，16-不可转换储蓄股票(意大利)，17-普通股(德国)，18-合订证券(澳大利亚)，19-追踪股票(美国)，20-组合证券，21-优先股(澳大利亚德国)。

### AlternateListing (另类交易)

另类交易(AlternateListing)与(CT_SystemConst)表中的DM字段关联，令LB=2572，得到另类交易的具体描述：1-ADR，2-CERT，3-FGN，4-GDR，5-NVDR，6-PDR，7-RDR，8-SDR，9-TDR，10-CDR。

### RegionCode (国家及地区代码)

国家及地区代码(RegionCode)：与“国家城市代码表(LC_AreaCode)”中的“地区内部编码(AreaInnerCode)”关联，得到具体信息。

### RegionCode_ND (次日国家及地区代码)

次日国家及地区代码(RegionCode_ND)：与“国家城市代码表(LC_AreaCode)”中的“地区内部编码(AreaInnerCode)”关联，得到具体信息。

### IfPublish (是否出版)

是否出版(IfPublish)，该字段固定常量以下常量：1-是；0-否

## SQL示例

```sql
-- 查询 MSCI指数成份当日基本信息 数据
SELECT *
FROM index_mscisecuinfo
