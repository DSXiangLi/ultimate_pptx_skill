...
| 属性 | 值 |
|------|-----|
| 表名 | `Bond_SCHIndexCMPT` |
| MySQL表名 | `bond_schindexcmpt` |
| 中文名 | 上清所指数样本券及构成 |
...
1.收录了上清所发布的银行间信用债综合指数(SCH00100)、银行间高等级信用债指数(SCH00200)、银行间中高等级信用债指数(SCH00300)、银行间高收益信用债指数(SCH00400)、银行间区域(上海)信用债指数(SCH00500)、非公开定向债务融资工具综合指数(SCH00600)、信用债综合指数(SCH00700)七大主指数及相应的待偿期分段子指数样本券构成及信息。
2.历史数据：2016年2月至今
3.数据源：上海清算所
...
| 3 | `MaturityDesc` | 待偿期限(类)_指数 | number(10) | ✓ | 99.98% | 待偿期限(类)_指数(MaturityDesc)与(CT_SystemConst)表中的DM字段关联，令LB = 118... |
...
| 6 | `InnerCode` | 样本券内部编码 | number(10) | ✗ | 100.0% | 样本券内部编码（InnerCode）：与“证券主表（SecuMain）”中的“证券内部编码（InnerCode）”关联，... |
| 7 | `MarketDesc` | 流通场所 | number(10) | ✓ | 100.0% | 流通场所(MarketDesc)与(CT_SystemConst)表中的DM字段关联，令LB = 201，得到流通场所的... |
| 8 | `RemainMaturity` | 待偿期(年)_样本券 | number(18,6) | ✓ | 100.0% |  |
| 9 | `FullPrice` | 全价(元) | number(18,6) | ✓ | 100.0% |  |
| 10 | `CleanPrice` | 净价(元) | number(18,6) | ✓ | 100.0% |  |
| 11 | `Yield` | 收益率(%) | number(18,6) | ✓ | 100.0% |  |
| 12 | `ModifiedDuration` | 修正久期 | number(18,6) | ✓ | 100.0% |  |
| 13 | `TrueConvexity` | 凸性 | number(18,6) | ✓ | 100.0% |  |
| 14 | `BasicPointValue` | 基点价值 | number(18,6) | ✓ | 100.0% |  |
