...
1.记录香港互认基金和香港ETF，每日单位基金净值以及复权单位净值和净值日增长率，及所适用的币种等数据。
2.历史数据：2008年7月起-至今。
3.数据来源：聚源计算而得。
...
| 4 | `UnitNV` | 单位净值 | number(9,4) | ✓ | 100.0% |  |
| 5 | `AccuUnitNV` | 累计净值 | number(9,4) | ✓ | 6.87% |  |
| 6 | `UnitNVRestored` | 复权单位净值 | number(19,6) | ✓ | 100.0% |  |
