...
| 属性 | 值 |
|------|-----|
| 表名 | `MF_FundSortinoRatio` |
| MySQL表名 | `mf_fundsortinoratio` |
| 中文名 | 基金索提诺比率 |
...
1.内容说明：基金收益评价指标，表示投资组合每承受一单位下行风险产生的超额报酬。
2.数据范围：2006年2月起-至今。
3.信息来源：根据基金每周最后交易日公布的复权单位净值、一年期国债收益率计算而得。
公式为：
索提诺比率=(基金平均周收益率-无风险收益率/52)/基金下行风险，
索提诺比率(年化)=SQRT(52)*{(基金平均周收益率-无风险收益率/52)/基金下行风险}=(基金平均周收益率*52-无风险收益率)/基金年化下行风险，
基金下行风险={∑[MiN（0，Ri-Rf/52)^2]/N}^0.5，
基金年化下行风险={∑[MiN（0，Ri-Rf/52)^2]/N}^0.5*sqrt(52)，
其中:
Ri表示基金周收益率，Rf为样本首日一年期国债收益率，N表示样本区间所含周数，
收益率的样本首日需大于等于截止日期往前追溯相应周期的日历日期加一天，一年（含）以上的周期为年化索提诺比率
