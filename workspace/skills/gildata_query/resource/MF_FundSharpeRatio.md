...
| 属性 | 值 |
|------|-----|
| 表名 | `MF_FundSharpeRatio` |
| MySQL表名 | `mf_fundsharperatio` |
| 中文名 | 基金夏普比率 |
...
1.内容说明：基金收益评价指标，表示基金每承受一单位总风险产生的超额报酬。此表对于转型基金转型前后作为一只处理。
2.数据范围：2006年2月起-至今。
3.信息来源：根据基金每周最后交易日公布的复权单位净值、一年期国债收益率计算而得。
公式为：
夏普比率=(基金平均周收益率-无风险收益率/52)/基金收益率标准差，(IndexCycle=1-一个月/3-三个月/6-六个月时，采用此公式直接计算不进行年化处理），
夏普比率(年化)=sqrt(52)*{(基金平均周收益率-无风险收益率/52)/基金收益率标准差}，(IndexCycle<>1-一个月/3-三个月/6-六个月时，采用此公式计算年化指标值），
其中：
无风险收益率为样本首日的一年期国债到期收益率，收益率的样本首日需大于等于截止日期往前追溯相应周期的日历日期加一天。
基金平均周收益率为时间周期内每周最后交易日的周收益率(未剔除首个不完整周区间）集合的算术平均值。
基金收益率标准差为时间周期内每周最后交易日的周收益率(未剔除首个不完整周区间）集合的标准差值。
注：
周期(含)一年以上的指标值为年化夏普比率。周期一年以下的指标值为未年化夏普比率。
