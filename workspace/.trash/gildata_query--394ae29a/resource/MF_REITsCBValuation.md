...
**中文名**: 中债基础设施基金(REITs)估值

## 基本信息

| 属性 | 值 |
|------|-----|
| 表名 | `MF_REITsCBValuation` |
| MySQL表名 | `mf_reitscbvaluation` |
| 中文名 | 中债基础设施基金(REITs)估值 |
...
1.内容说明：用于记录REITs估值数据，包括估值价格、估值收益率等。
2.数据范围：2021-8-13 至今
...
| 3 | `InnerCode` | 内部编码 | number(10) | ✗ | 100.0% | 基金内部编码（InnerCode）:与“证券主表（SecuMain）”中的“证券内部编码（InnerCode）”关联，得... |
| 4 | `SecuMarket` | 证券市场 | number(10) | ✓ | 100.0% | 证券市场(SecuMarket)与(CT_SystemConst)表中的DM字段关联，令LB=201，得到证券市场的具体... |
| 5 | `ValuePrice` | 中债估值价格 | number(18,8) | ✓ | 100.0% |  |
| 6 | `VPYield` | 中债估值收益率(%) | number(18,8) | ✓ | 100.0% |  |
...
```sql
-- 查询 中债基础设施基金(REITs)估值 数据
SELECT *
FROM mf_reitscbvaluation
