...
1.记录辉立证券、耀才证券在新股上市之前一个营业日在自营交易场新股的成交情况。包含主要字段有：交易券商、交易日期、上市日期、收盘价、涨跌幅、成交股数、成交金额等。
2.数据范围：2017-02-2024-12
3.数据来源：辉立证券、耀才证券。
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `InnerCode` | 证券内部代码 | number(10) | ✗ | 100.0% | 证券内部代码（InnerCode）：与“港股证券主表（HK_SecuMain）”中的“证券内部编码（InnerCode）... |
| 3 | `TradingDay` | 交易日期 | date | ✗ | 100.0% |  |
| 4 | `ListedDate` | 上市日期 | date | ✓ | 100.0% |  |
| 5 | `TradStockbroker` | 交易券商 | varchar2(20) | ✗ | 100.0% | 交易券商（TradStockbroker）：为提供港股暗盘交易的券商，目前有两家，分别是辉立证券和耀才证券。 |
| 6 | `IssuePrice` | 发行价(元) | number(19,4) | ✓ | 99.95% |  |
| 7 | `OpenPrice` | 开盘价(元) | number(19,4) | ✓ | 49.92% |  |
| 8 | `HighPrice` | 最高价(元) | number(19,4) | ✓ | 99.84% |  |
| 9 | `LowPrice` | 最低价(元) | number(19,4) | ✓ | 99.84% |  |
| 10 | `ClosePrice` | 收盘价(元) | number(19,4) | ✓ | 100.0% |  |
| 11 | `ChangeOfPrice` | 涨跌(元) | number(19,4) | ✓ | 100.0% |  |
| 12 | `ChangePCT` | 涨跌幅(%) | number(18,4) | ✓ | 100.0% |  |
| 13 | `TransactionNO` | 成交盘数 | number(14,6) | ✓ | 49.82% |  |
| 14 | `TurnoverVolume` | 成交股数(股) | number(14,0) | ✓ | 99.84% |  |
| 15 | `TurnoverValue` | 成交金额(元) | number(19,4) | ✓ | 99.84% |  |
| 16 | `Lot` | 交易单位(股/手) | number(18,4) | ✓ | 100.0% |  |
| 17 | `MinPriceChg` | 最小变动价格 | number(19,4) | ✓ | 99.95% |  |
| 18 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 99.63% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币... |
...
### TradStockbroker (交易券商)

交易券商（TradStockbroker）：为提供港股暗盘交易的券商，目前有两家，分别是辉立证券和耀才证券。
