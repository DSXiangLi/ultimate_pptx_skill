...
| 属性 | 值 |
|------|-----|
| 表名 | `HK_DirectorEquity` |
| MySQL表名 | `hk_directorequity` |
| 中文名 | 港股董事权益 |
...
1.记录港股董事的相关权益信息，包括董事名称、权益性质、股东身份、以不同身份持有的股票数量等内容。
2.数据范围：2001年至今。
3.信息来源：港交所。
...
| 7 | `EquityType` | 权益类别 | number(10) | ✓ | 100.0% | 权益类别(EquityType)与(CT_SystemConst)表中的DM字段关联，令LB = 1341，得到权益类别... |
| 8 | `EquityCharacter` | 权益性质 | number(10) | ✓ | 100.0% | 权益性质(EquityCharacter)与(CT_SystemConst)表中的DM字段关联，令LB = 1342，得... |
| 9 | `Unit` | 单位 | number(10) | ✓ | 0.31% | 单位(Unit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到单位的具体描述：1000... |
| 10 | `EquityStatus` | 权益身份 | number(10) | ✓ |  |  |
| 11 | `EquityStatusDesc` | 权益身份描述 | varchar2(200) | ✓ | 74.98% |  |
| 12 | `RelatingEquityStatus` | 相关权益身份 | number(10) | ✓ |  |  |
| 13 | `RelatingEquityStatusDesc` | 相关权益身份描述 | varchar2(200) | ✓ | 21.97% |  |
| 14 | `EquityVolume` | 权益总数(股) | number(18,2) | ✓ | 99.79% |  |
| 15 | `RatioInTotalShares` | 占已发行股份比例 | number(18,8) | ✓ | 99.49% |  |
| 16 | `HoldSum` | 1、股份权益总数(股) | number(18,2) | ✓ | 77.86% |  |
| 17 | `HSInTotalShares` | 占已发行股份比例 | number(18,8) | ✓ | 77.75% |  |
| 18 | `PersonalEquity` | #个人权益(股) | number(18,2) | ✓ | 48.96% |  |
| 19 | `FamilyEquity` | #家族权益(股) | number(18,2) | ✓ | 8.46% |  |
| 20 | `CompanyEquity` | #公司权益(股) | number(18,2) | ✓ | 26.28% |  |
| 21 | `OtherEquity` | #其他权益(股) | number(18,2) | ✓ | 9.89% |  |
| 22 | `RelatingEquityHoldSum` | 2、相关股份权益总数(股) | number(18,2) | ✓ | 42.04% |  |
| 23 | `REHSInTotalShares` | 占已发行股份比例 | number(18,8) | ✓ | 41.95% |  |
| 24 | `StockrightEquity` | #购股权权益(股) | number(18,2) | ✓ | 38.64% |  |
| 25 | `WarrantEquity` | #认股权证权益(股) | number(18,2) | ✓ | 0.53% |  |
| 26 | `ConvertibleBondEquity` | #可换股债券权益(股) | number(18,2) | ✓ | 0.58% |  |
| 27 | `OtherRelatingEquity` | #其他相关权益(股) | number(18,2) | ✓ | 2.88% |  |
| 28 | `Statement` | 备注 | clob | ✓ | 21.3% |  |
