...
| 属性 | 值 |
|------|-----|
| 表名 | `DZ_AShareCDRTransRatio` |
| MySQL表名 | `dz_asharecdrtransratio` |
| 中文名 | CDR与基础证券转换 |
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `InnerCode` | 存托凭证内部编码 | number(10) | ✗ | 100.0% | 存托凭证内部编码(InnerCode)：与“证券主表（SecuMain）”中的“证券内部编码（InnerCode）”关联... |
| 3 | `DRSharesNum` | 存托凭证份数 | number(18,2) | ✓ | 100.0% | 该存托凭证份数的有效时间仅为当前生效日期对应的存托凭证份数 |
| 4 | `BasicSecuSharesNum` | 基础证券股数 | number(18,2) | ✓ | 100.0% | 该基础证券股数的有效时间仅为当前生效日期对应的基础证券股数 |
| 5 | `BasicSecuType` | 基础证券种类 | varchar2(50) | ✓ | 100.0% |  |
| 6 | `TransRatio` | 转换比例 | number(18,6) | ✓ | 100.0% | 转换比例(TransRatio)：基础证券股数/存托凭证份数 |
...
| 12 | `BasicSecuParValue` | 基础股票面值 | number(25,10) | ✓ | 100.0% |  |
| 13 | `BasicSecuParValueUnit` | 基础股票面值单位 | number(10) | ✓ | 100.0% | 基础股票面值单位(BasicSecuParValueUnit)与(CT_SystemConst)表中的DM字段关联，令L... |
