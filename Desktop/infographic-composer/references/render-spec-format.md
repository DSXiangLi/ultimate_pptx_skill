...
6. 交付时附一段 ≤5 行的"修改指南"：告诉用户最可能想改的 3 个字段在哪——**修改指南仅供用户参考，不出现在 render spec 交付物中**。
7. **颜色图例/配色角色表不出现在交付物中**——这是系统内部参考，不是给信息图读者的内容。style 槽位表中标注为"内部参考"的图例行，仅在 skill 工作流中使用，组装 spec 时跳过。
8. **spec 必须写入文件**，文件命名格式 `<场景编号>-<拓扑>-<风格>-<日期>.md`（如 `S1-hero-perler-20260612.md`），存放于当前工作目录。命名应一目了然：从文件名即可读出场景、拓扑、风格、日期。
9. **写出文件后必须执行人工确认**（SKILL.md Step 6），未经用户确认的 spec 不得送入图片生成。确认重点：风格搭配、标题文字、正文逐字、风险提示（CAUTION）、页脚信息（META）。
