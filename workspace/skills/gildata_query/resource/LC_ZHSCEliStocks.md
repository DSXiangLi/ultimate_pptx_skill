...
1.收录深港通业务中，各类交易（可买入及卖出、只可卖出、可进行保证金交易、可进行担保卖空）的合资格股票的最新清单以及变动情况。
2.历史数据：2016年12月起-至今
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `TradingType` | 交易类型 | number(10) | ✗ | 100.0% | 交易类型(TradingType)与(CT_SystemConst)表中的DM字段关联，令LB = 1844 and D... |
...
| 8 | `OutDate` | 截止日期 | date | ✓ | 41.28% |  |
| 9 | `Flag` | 是否交易标的 | number(10) | ✗ | 100.0% | 是否交易标的（Flag），该字段固定以下常量：1-是，2-否 |
| 10 | `CCASSCode` | CCASS股份编码 | varchar2(50) | ✓ | 88.27% |  |
| 11 | `ParValue` | 面值(人民币) | varchar2(50) | ✓ | 88.27% |  |
