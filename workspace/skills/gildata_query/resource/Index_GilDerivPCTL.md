...
| 属性 | 值 |
|------|-----|
| 表名 | `Index_GilDerivPCTL` |
| MySQL表名 | `index_gilderivpctl` |
| 中文名 | 聚源指数衍生指标分位数 |
...
1.内容说明：本表记录聚源自编股票指数衍生指标的百分位，包括滚动市盈率、市净率等指标每天所处的历史分位情况；以及30%分位和70%等分位对应的分位值。当滚动市盈率、市净率为负时，不会计算相应的分位数。
2.数据范围：指数基日-至今
3.信息来源：聚源计算得到
...
| 5 | `StatisPeriod` | 指标统计周期 | number(10) | ✗ | 100.0% | 指标统计周期(StatisPeriod)与(CT_SystemConst)表中的DM字段关联，令LB = 2149 AN... |
...
指标统计周期(StatisPeriod)与(CT_SystemConst)表中的DM字段关联，令LB = 2149 AND DM in (3,6,12,24,36,60,120,998,999)，得到指标统计周期的具体描述：
