...
| 属性 | 值 |
|------|-----|
| 表名 | `Index_MSCISecuDriv` |
| MySQL表名 | `index_mscisecudriv` |
| 中文名 | MSCI指数成份衍生指标 |
...
内容说明：收录MSCI指数成份月度财务估值衍生指标数据，包括估值指标（ROE、市盈率、市净率、市现率、市销率、股息率）、财务指标（每股收益、每股账面价值、每股股息历史和预测及增长率数据）。
数据范围：2003年至今
信息来源：MSCI
...
| 5 | `RIC` | 路孚特代码 | varchar2(100) | ✓ | 99.8% |  |
| 6 | `InnerCode` | 证券内部编码 | number(10) | ✓ | 30.64% | 证券内部编码(InnerCode)：关联不同主表，查询证券代码、证券简称等基本信息。当0<SecuInnerCode<=... |
| 7 | `SecuMarket` | 证券市场 | number(10) | ✓ | 100.0% | 证券市场(SecuMarket)与(CT_SystemConst)表中的DM字段关联，令LB=201，得到证券市场的具体... |
| 8 | `EPS` | 每股收益(元) | number(19,3) | ✓ | 98.73% |  |
| 9 | `CashEPS` | 每股现金收益 | number(19,3) | ✓ | 91.56% |  |
| 10 | `CCY_Earnings` | 币种_收益 | number(10) | ✓ | 98.82% | 币种_收益(CCY_Earnings)与(CT_SystemConst)表中的DM字段关联，令LB=1548，得到币种_... |
| 11 | `Unit_Earnings` | 单位_收益 | number(19,3) | ✓ | 98.82% |  |
| 12 | `EndDate_EPS` | 每股收益财报期 | date | ✓ | 98.82% |  |
| 13 | `IfMerged_Earnings` | 是否并表_收益 | number(10) | ✓ | 99.11% | 是否并表_收益(IfMerged_Earnings)，该字段固定常量以下常量：1-是；0-否 |
| 14 | `BookValuePerShare` | 每股账面价值 | number(19,3) | ✓ | 98.73% |  |
| 15 | `CCY_BV` | 币种_账面价值 | number(10) | ✓ | 98.81% | 币种_账面价值(CCY_BV)与(CT_SystemConst)表中的DM字段关联，令LB=1548，得到币种_账面价值... |
| 16 | `Unit_BV` | 单位_账面价值 | number(19,3) | ✓ | 98.81% |  |
| 17 | `EndDate_BV` | 账面价值财报期 | date | ✓ | 98.81% |  |
| 18 | `IfMerged_BV` | 是否并表_账面价值 | number(10) | ✓ | 99.11% | 是否并表_账面价值(IfMerged_BV)，该字段固定常量以下常量：1-是；0-否 |
| 19 | `DividendPerShare` | 每股股息(元) | number(19,3) | ✓ | 98.24% |  |
| 20 | `PurifiedDPS` | 每股股息(purified) | number(19,3) | ✓ | 37.69% |  |
| 21 | `CCY_Dividend` | 股息币种 | number(10) | ✓ | 98.23% | 股息币种(CCY_Dividend)与(CT_SystemConst)表中的DM字段关联，令LB=1548，得到股息币种... |
| 22 | `Unit_Dividend` | 单位_股息 | number(19,3) | ✓ | 98.24% |  |
| 23 | `LastExDiviDate` | 最新除息日 | date | ✓ | 97.79% |  |
| 24 | `SalesPerShare` | 每股营收 | number(19,3) | ✓ | 93.0% |  |
| 25 | `CCY_Sales` | 币种_营业收入 | number(10) | ✓ | 93.39% | 币种_营业收入(CCY_Sales)与(CT_SystemConst)表中的DM字段关联，令LB=1548，得到币种_营... |
| 26 | `Unit_Sales` | 单位_营业收入 | number(19,3) | ✓ | 94.58% |  |
| 27 | `EndDate_SPS` | 每股营收财报期 | date | ✓ | 93.43% |  |
| 28 | `IfMerged_Sales` | 是否并表_营业收入 | number(10) | ✓ | 99.11% | 是否并表_营业收入(IfMerged_Sales)，该字段固定常量以下常量：1-是；0-否 |
| 29 | `IfEstimated` | 是否预测 | number(10) | ✓ | 99.11% | 是否预测(IfEstimated)，该字段固定常量以下常量：1-是；0-否 |
| 30 | `EPS0_EndDate` | 每股收益年度0 | date | ✓ | 97.18% |  |
| 31 | `EPS0` | 每股收益(年度0) | number(19,3) | ✓ | 97.17% |  |
| 32 | `EPS1_EndDate` | 每股收益年度Y-1 | date | ✓ | 96.23% |  |
| 33 | `EPS1` | 每股收益(年度Y-1) | number(19,3) | ✓ | 96.23% |  |
| 34 | `EPS2_EndDate` | 每股收益年度Y-2 | date | ✓ | 94.65% |  |
| 35 | `EPS2` | 每股收益(年度Y-2) | number(19,3) | ✓ | 94.65% |  |
| 36 | `EPS3_EndDate` | 每股收益年度Y-3 | date | ✓ | 92.73% |  |
| 37 | `EPS3` | 每股收益(年度Y-3) | number(19,3) | ✓ | 92.72% |  |
| 38 | `EPS4_EndDate` | 每股收益年度Y-4 | date | ✓ | 89.71% |  |
| 39 | `EPS4` | 每股收益(年度Y-4) | number(19,3) | ✓ | 89.7% |  |
| 40 | `DPS0_EndDate` | 每股股息年度0 | date | ✓ | 43.18% |  |
| 41 | `DPS0` | 每股股息(年度0) | number(19,3) | ✓ | 43.18% |  |
| 42 | `DPS1_EndDate` | 每股股息年度Y-1 | date | ✓ | 42.41% |  |
| 43 | `DPS1` | 每股股息(年度Y-1) | number(19,3) | ✓ | 42.41% |  |
| 44 | `DPS2_EndDate` | 每股股息年度Y-2 | date | ✓ | 41.19% |  |
| 45 | `DPS2` | 每股股息(年度Y-2) | number(19,3) | ✓ | 41.19% |  |
| 46 | `DPS3_EndDate` | 每股股息年度Y-3 | date | ✓ | 39.67% |  |
| 47 | `DPS3` | 每股股息(年度Y-3) | number(19,3) | ✓ | 39.67% |  |
| 48 | `DPS4_EndDate` | 每股股息年度Y-4 | date | ✓ | 37.98% |  |
| 49 | `DPS4` | 每股股息(年度Y-4) | number(19,3) | ✓ | 37.98% |  |
| 50 | `SPS0_EndDate` | 每股营收年度0 | date | ✓ | 91.81% |  |
| 51 | `SPS0` | 每股营收(年度0) | number(19,3) | ✓ | 91.8% |  |
| 52 | `SPS1_EndDate` | 每股营收年度Y-1 | date | ✓ | 90.89% |  |
| 53 | `SPS1` | 每股营收(年度Y-1) | number(19,3) | ✓ | 90.88% |  |
| 54 | `SPS2_EndDate` | 每股营收年度Y-2 | date | ✓ | 89.37% |  |
| 55 | `SPS2` | 每股营收(年度Y-2) | number(19,3) | ✓ | 89.36% |  |
| 56 | `SPS3_EndDate` | 每股营收年度Y-3 | date | ✓ | 87.54% |  |
| 57 | `SPS3` | 每股营收(年度Y-3) | number(19,3) | ✓ | 87.53% |  |
| 58 | `SPS4_EndDate` | 每股营收年度Y-4 | date | ✓ | 84.67% |  |
| 59 | `SPS4` | 每股营收(年度Y-4) | number(19,3) | ✓ | 84.67% |  |
| 60 | `ROE` | 净资产收益率 | number(19,3) | ✓ | 97.91% |  |
| 61 | `PB` | 市净率 | number(19,3) | ✓ | 98.38% |  |
| 62 | `PE` | 市盈率 | number(19,3) | ✓ | 98.37% |  |
| 63 | `PCF` | 市现率 | number(19,3) | ✓ | 91.28% |  |
| 64 | `PS` | 市销率 | number(19,3) | ✓ | 92.66% |  |
| 65 | `DividendRatio` | 股息率 | number(19,3) | ✓ | 97.93% |  |
| 66 | `PayoutRatio` | 股息发放率 | number(19,3) | ✓ | 49.44% |  |
| 67 | `PurifiedDividendRatio` | 股息率(purified) | number(19,3) | ✓ | 37.48% |  |
| 68 | `CCY_EPS_SPS_FiveYear` | 每股收益和每股营收近5年币种 | number(10) | ✓ | 90.44% | 每股收益和每股营收近5年币种(CCY_EPS_SPS_FiveYear)与(CT_SystemConst)表中的DM字段... |
| 69 | `EstFlag_EPS_FiveYear` | 每股收益近5年预估标志 | number(10) | ✓ | 1.99% | 每股收益近5年预估标志(EstFlag_EPS_FiveYear)，该字段固定常量以下常量：1-是；0-否 |
| 70 | `EstFlag_SPS_FiveYear` | 每股营收近5年预估标志 | number(10) | ✓ | 89.69% | 每股营收近5年预估标志(EstFlag_SPS_FiveYear)，该字段固定常量以下常量：1-是；0-否 |
| 71 | `InternalGrowth` | 内含增长率(%) | number(19,3) | ✓ | 96.83% |  |
| 72 | `EPSGrowthFiveYear` | 每股收益近5年增长率(%) | number(19,3) | ✓ | 2.0% |  |
| 73 | `SPSGrowthFiveYear` | 每股营收近5年增长率(%) | number(19,3) | ✓ | 87.78% |  |
| 74 | `ROE_Flag` | 净资产收益率标志 | number(10) | ✓ | 99.57% | 净资产收益率标志(ROE_Flag)，该字段固定常量以下常量：1-是；0-否 |
| 75 | `CCY_DPS_FiveYear` | 每股股息近5年币种 | number(10) | ✓ | 42.91% | 每股股息近5年币种(CCY_DPS_FiveYear)与(CT_SystemConst)表中的DM字段关联，令LB=15... |
| 76 | `AnnualizedDPSOneYear` | 每股股息近1年年化增长率(%) | number(19,3) | ✓ | 48.15% |  |
| 77 | `AnnualizedDPSFiveYear` | 每股股息近5年年化增长率(%) | number(19,3) | ✓ | 45.24% |  |
