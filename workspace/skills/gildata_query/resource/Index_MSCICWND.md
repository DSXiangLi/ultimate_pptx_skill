...
内容说明：收录MSCI指数次日成份开盘参考价、开盘权重及股本、价格调整因子等信息，提供未来三个交易日的预估数据。
数据范围：2024年至今
信息来源：MSCI
...
| 4 | `IndexCode` | 指数内部编码 | number(10) | ✗ | 100.0% | 指数内部编码(IndexCode)：与“证券主表(SecuMain)”中的“证券内部编码(InnerCode)”关联，得... |
| 5 | `MSCode` | MS成份券内码 | number(10) | ✗ | 100.0% | MS成份券内码(MSCode)：与“MSCI指数成份次日基本信息(Index_MSCISecuInfoND)”中的“MS... |
| 6 | `InnerCode` | 证券内部编码 | number(10) | ✓ | 98.23% | 证券内部编码(InnerCode)：关联不同主表，查询证券代码、证券简称等基本信息。当0<SecuInnerCode<=... |
| 7 | `RIC` | 路孚特代码 | varchar2(100) | ✓ | 91.56% |  |
| 8 | `SecuMarket` | 证券市场 | number(10) | ✓ | 91.56% | 证券市场(SecuMarket)与(CT_SystemConst)表中的DM字段关联，令LB=201，得到证券市场的具体... |
| 9 | `OpenWeightedRatio` | 开盘权重(%) | number(22,13) | ✓ | 100.0% |  |
| 10 | `FullMCAdjFactor` | 总市值调整因子 | number(22,9) | ✓ | 100.0% |  |
| 11 | `InitialMV` | 初始市值(美元) | number(28,6) | ✓ | 100.0% |  |
| 12 | `ReferPrice` | 调整后开盘参考价 | number(22,5) | ✓ | 99.25% |  |
| 13 | `TotalShares` | 总股本(股) | number(22,4) | ✓ | 99.25% |  |
| 14 | `AdjustedShares` | 计算用股本(股) | number(22,4) | ✓ | 99.25% |  |
| 15 | `InitialTMV` | 初始总市值(美元) | number(28,6) | ✓ | 99.25% |  |
| 16 | `CurrencyCode` | 币种 | number(10) | ✓ | 99.2% | 币种(CurrencyCode)与(CT_SystemConst)表中的DM字段关联，令LB=1548，得到币种的具体描... |
| 17 | `ExchangeRate` | 汇率 | number(22,15) | ✓ | 99.25% |  |
| 18 | `AdjustType` | 调整类型 | number(10) | ✓ | 0.81% | 调整类型(AdjustType)与(CT_SystemConst)表中的DM字段关联，令LB=2554，得到调整类型的具... |
| 19 | `IfAdjusted` | 是否调整价格 | number(10) | ✓ | 99.24% | 是否调整价格(IfAdjusted)，该字段固定常量以下常量：1-是；0-否 |
| 20 | `PriceAdjFactor` | 价格调整因子 | number(19,6) | ✓ | 33.13% |  |
| 21 | `IfRebalancing` | 是否调样中 | number(10) | ✓ | 99.25% | 是否调样中(IfRebalancing)，该字段固定常量以下常量：1-是；0-否 |
