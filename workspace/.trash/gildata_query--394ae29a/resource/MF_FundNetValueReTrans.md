...
| 属性 | 值 |
|------|-----|
| 表名 | `MF_FundNetValueReTrans` |
| MySQL表名 | `mf_fundnetvalueretrans` |
| 中文名 | 公募基金复权净值(转型) |
...
【转型处理：对转型基金处理收益率和净值时，会将转型前后数据同步计算。本表只收录了转型后的内码。】
...
| 7 | `NVDailyGrowthRate` | 单位净值日增长率 | number(18,6) | ✓ | 91.14% |  |
| 8 | `NVRDailyGrowthRate` | 复权单位净值日增长率(%) | number(18,6) | ✓ | 99.92% |  |
