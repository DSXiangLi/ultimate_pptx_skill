...
| 属性 | 值 |
|------|-----|
| 表名 | `LC_IssuanceExamination` |
| MySQL表名 | `lc_issuanceexamination` |
| 中文名 | 发行审核表 |
...
1.收录股票发行审核委员会历年来对各拟发行人的审核情况，包括审核会议召开日期、参会委员、是否通过等信息。
2.数据范围：2003-12-26至今
3.信息来源：中国证监会、临时公告等
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `InitialInfoPublDate` | 首次信息发布日期 | date | ✓ | 91.92% |  |
| 3 | `InfoPublDate` | 信息发布日期 | date | ✗ | 100.0% |  |
| 4 | `InfoSource` | 信息来源 | varchar2(200) | ✓ | 99.99% |  |
| 5 | `IndustryStd` | 行业分类标准 | number(10) | ✓ | 36.45% | 行业分类标准（IndustryStd），当为1时，代表CSRC行业分类；当为22时，代表证监会行业分类2012版。 |
| 6 | `CSRCIndustryNum` | 行业类别 | number(10) | ✓ | 34.14% | 证监会行业类别(CSRCIndustryNum)：与“行业类别表（CT_IndustryType）”中的“行业内部编码（... |
| 7 | `CSRCIndustryCode` | 行业代码 | varchar2(10) | ✓ | 33.9% |  |
| 8 | `Issuer` | 发行人 | varchar2(200) | ✗ | 100.0% |  |
| 9 | `RegAddr` | 注册地址 | varchar2(200) | ✓ | 37.97% |  |
| 10 | `Province` | 所在省市 | varchar2(20) | ✓ | 37.95% |  |
| 11 | `ProvinceCode` | 省市代码 | number(10) | ✓ | 37.95% | 省市代码(ProvinceCode)：可关联系统常量表中的DM字段关联，令LB = 1145，得到地区代码的具体描述；也... |
| 12 | `BusinessMajor` | 主营产品及业务 | varchar2(1000) | ✓ | 36.49% |  |
| 13 | `Committee` | 委员会 | varchar2(50) | ✓ | 97.68% |  |
| 14 | `CommitteeCode` | 委员会代码 | number(10) | ✓ | 97.68% | 委员会代码(CommitteeCode)与(CT_SystemConst)表中的DM字段关联，令LB = 1553，得到... |
| 15 | `MeetingNumber` | 会议列次 | number(10) | ✗ | 100.0% |  |
| 16 | `ExaminationMeetingDate` | 审核会议召开日 | date | ✓ | 98.21% |  |
| 17 | `PresentCommitteeman` | 参会委员 | varchar2(500) | ✓ | 65.68% |  |
| 18 | `PresentCommitteCode` | 参会委员代码 | number(10) | ✓ |  |  |
| 19 | `ResultPublDate` | 审核结果公告日 | date | ✓ | 99.93% |  |
| 20 | `IfPassed` | 审核是否通过 | number(10) | ✓ | 99.93% | 审核是否通过(IfPassed)与(CT_SystemConst)表中的DM字段关联，令LB = 999 AND DM ... |
| 21 | `RegiResultPublDate` | 注册结果公告日 | date | ✓ | 12.03% |  |
| 22 | `RegisterIfPassed` | 注册是否通过 | number(10) | ✓ | 12.03% | 注册是否通过(RegisterIfPassed)与(CT_SystemConst)表中的DM字段关联，令LB = 169... |
| 23 | `PrecontractedDisclosureD` | 发行申报稿预披露日 | date | ✓ | 37.88% |  |
| 24 | `MeetingDraftADD` | 上会稿预披露日 | date | ✓ | 10.1% |  |
| 25 | `RegisterDraftADD` | 注册稿预披露日 | date | ✓ | 8.43% |  |
...
| 32 | `IssueShares` | 发行股数(万股) | number(18,4) | ✓ | 41.7% |  |
| 33 | `NewSharesVol` | 其中:新股发行数量(万股) | number(18,4) | ✓ | 17.03% |  |
| 34 | `ListStandard` | 上市标准 | number(10) | ✓ | 10.12% | 上市标准(ListStandard)与(CT_SystemConst)表中的DM字段关联，令LB = 2206，得到上市... |
| 35 | `ParValue` | 每股面值 | number(25,10) | ✓ | 36.67% |  |
| 36 | `ParValueCurrencyUnit` | 每股面值货币单位 | number(10) | ✓ | 36.67% | 每股面值货币单位(ParValueCurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB... |
| 37 | `PlannedProceeds` | 拟募集资金金额(万元) | number(19,4) | ✓ | 36.07% |  |
| 38 | `TotalSharesBefore` | 发行前总股本(万股) | number(18,4) | ✓ | 37.72% |  |
| 39 | `TotalSharesAfter` | 发行后总股本(万股) | number(18,4) | ✓ | 37.56% |  |
| 40 | `PreparedListExchange` | 拟上市地 | varchar2(20) | ✓ | 37.44% |  |
| 41 | `ListExchangeCode` | 拟上市地代码 | number(10) | ✓ | 37.44% | 拟上市地代码(ListExchangeCode)与(CT_SystemConst)表中的DM字段关联，令LB = 201... |
| 42 | `Sponsor` | 保荐机构 | varchar2(600) | ✓ | 37.93% |  |
| 43 | `SponsorCode` | 保荐机构代码 | number(10) | ✓ |  |  |
| 44 | `EventDesc` | 事件描述 | varchar2(200) | ✓ | 99.15% |  |
| 45 | `CommitteeFeedback` | 委员会意见 | clob | ✓ | 54.81% |  |
| 46 | `IfImplemented` | 是否实施 | number(10) | ✓ | 100.0% | 是否实施(IfImplemented)：与“系统常量表（CT_SystemConst）”中的“代码（DM）”关联，令“L... |
| 47 | `Remark` | 备注说明 | clob | ✓ | 1.78% |  |
| 48 | `UpdateTime` | 更新时间 | date | ✗ |  |  |
| 49 | `JSID` | JSID | number(19) | ✗ |  |  |
| 50 | `IssuePrice` | 发行价格(元/股) | number(19,4) | ✓ | 0.81% |  |
| 51 | `OriginalSharesVol` | 其中:股东公开发售股份数量(万股) | number(18,4) | ✓ | 0.02% |  |
| 52 | `StrategicSharesVol` | 战略配售数量(万股) | number(18,4) | ✓ | 0.0% |  |
| 53 | `CoreStaffsStraSHVol` | 其中:高管、员工参与战略配售数量(万股) | number(18,4) | ✓ | 0.0% |  |
| 54 | `CoreStaffsStraSHVal` | 其中:高管、员工参与战略配售股份金额(万元) | number(19,4) | ✓ | 0.0% |  |
| 55 | `CoreStaffsStraSHRat` | 其中:高管、员工参与战略配售占比(%) | number(9,4) | ✓ | 0.0% |  |
| 56 | `SponsorStraSharesVol` | 其中:保荐机构及相关子公司参与战略配售数量(万股) | number(18,4) | ✓ | 0.0% |  |
| 57 | `SponsorStraSharesHVal` | 其中:保荐机构及相关子公司参与战略配售股份金额(万元) | number(19,4) | ✓ | 0.0% |  |
| 58 | `SponsorStraSharesRat` | 其中:保荐机构及相关子公司参与战略配售占比(%) | number(9,4) | ✓ | 0.0% |  |
| 59 | `OtherStraSHVol` | 其中:其他参与战略配售计划数量(万股) | number(18,4) | ✓ | 0.0% |  |
| 60 | `OtherStraSHVal` | 其中:其他参与战略配售计划金额(万元) | number(19,4) | ✓ | 0.01% |  |
| 61 | `OtherStraSHRat` | 其中:其他计划参与战略配售占比(%) | number(9,4) | ✓ | 0.0% |  |
| 62 | `FinancialAdviser` | 财务顾问 | varchar2(200) | ✓ | 1.4% |  |
| 63 | `FinancialAdviserCode` | 财务顾问代码 | number(10) | ✓ |  |  |
