...
1.内容说明：新建港股股东增减持表，记录港股主要股东的增减持股份记录信息。
2.数据范围：2000年至今。
3.信息来源：恒生聚源整理。
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `InnerCode` | 证券内部编码 | number(10) | ✗ | 100.0% | 内部代码（InnerCode）：与“港股证券主表（HK_SecuMain）”中的“证券内部编码（InnerCode）”关... |
| 3 | `SecuCode` | 证券代码 | varchar2(10) | ✓ | 100.0% |  |
| 4 | `SHNo` | 持有人编号 | number(10) | ✓ | 40.1% |  |
| 5 | `ChiHolderName` | 持有人姓名 | varchar2(200) | ✗ | 100.0% |  |
| 6 | `EventDate` | 事件日期 | date | ✗ | 100.0% |  |
| 7 | `IssuedShares` | 已发行股份(股) | number(18,2) | ✓ | 100.0% |  |
| 8 | `HoldSumBefEvent` | 事件前持股总数(股) | number(18,2) | ✗ | 100.0% |  |
| 9 | `HoldSumAfEvent` | 事件后持股总数(股) | number(18,2) | ✗ | 100.0% |  |
| 10 | `ChangeDirect` | 变动方向 | varchar2(10) | ✓ | 100.0% | 变动方向（ChangeDirect）：包括增持、减持、清仓减持三种类型。 |
| 11 | `ShareCategoryDesc` | 股份类别描述 | varchar2(200) | ✓ | 99.86% |  |
| 12 | `VariableQuantity` | 变动数量(股) | number(18,2) | ✓ | 100.0% |  |
| 13 | `AvePPSOnExchg` | 场内每股平均价(元) | number(18,9) | ✓ | 52.58% |  |
| 14 | `Currency` | 交易货币 | number(10) | ✓ | 92.06% | 交易货币(Currency)与(CT_SystemConst)表中的DM字段关联，令LB=1068，得到交易货币的具体描... |
