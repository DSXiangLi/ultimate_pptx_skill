...
| 属性 | 值 |
|------|-----|
| 表名 | `LC_STIBMainOperIncome` |
| MySQL表名 | `lc_stibmainoperincome` |
| 中文名 | 科创板主营业务构成 |
...
1.内容说明：收录科创板公司主营业务的收入来源、成本构成；主营业务收入、成本和利润与上年同期的比较。
2.数据范围：科创板上市至今
3.信息来源：科创板公司公告
...
| 5 | `DateType` | 日期类型 | number(10) | ✗ | 100.0% | 日期类型(DateType)与(CT_SystemConst)表中的DM字段关联，令LB = 1074 and DM I... |
...
| 8 | `Classification` | 分类方式 | number(10) | ✗ | 100.0% | 分类方式(Classification)与(CT_SystemConst)表中的DM字段关联，令LB = 1046 AN... |
| 9 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 100.0% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币... |
| 10 | `SerialNumber` | 序号 | number(10) | ✓ | 100.0% |  |
| 11 | `Project` | 经营项目名称 | varchar2(200) | ✗ | 100.0% |  |
| 12 | `Level` | 所属层级 | number(10) | ✓ | 100.0% | 所属层级（Level）展示各字段对应层级：如合计--0级；合计下面的一级明细项--1级；一级明细项对应的其中项--2级，... |
| 13 | `ParentName` | 上级科目名称 | varchar2(255) | ✓ | 83.94% | 上级科目名称（ParentName）展示各字段对应上级科目名称：如“合计”对应上级为空；“合计下面的一级明细项”对应上级... |
| 14 | `MainOperIncome` | 主营业务收入(元) | number(19,4) | ✓ | 99.26% |  |
| 15 | `MainOperCost` | 主营业务成本(元) | number(19,4) | ✓ | 62.3% |  |
| 16 | `MainOperProfit` | 主营业务利润(元) | number(19,4) | ✓ | 62.78% |  |
| 17 | `GrossProfit` | 毛利率 | number(14,8) | ✓ | 63.43% | 毛利率(GrossProfit)：优先取原文披露值，如未披露，则计算，计算公式=当主营业务收入>0时，（主营业务收入-主... |
| 18 | `MainOperIncoFormerYear` | 上年同期主营业务收入(元) | number(19,4) | ✓ | 6.26% |  |
| 19 | `MainOperCostFormerYear` | 上年同期主营业务成本(元) | number(19,4) | ✓ | 4.29% |  |
| 20 | `MainOperProfFormerYear` | 上年同期主营业务利润(元) | number(19,4) | ✓ | 4.24% |  |
| 21 | `GrossProfitFormerYear` | 上年同期毛利率 | number(14,8) | ✓ | 4.25% | 上年同期毛利率(GrossProfitFormerYear)：优先取原文披露值，如未披露，则计算，计算公式=当上年同期主... |
| 22 | `MainIncomeGrowRateYOY` | 主营业务收入同比 | number(18,4) | ✓ | 15.18% |  |
| 23 | `MainICostGrowRateYOY` | 主营业务成本同比 | number(18,4) | ✓ | 12.85% |  |
| 24 | `MainProfitGrowRateYOY` | 主营业务利润同比 | number(18,4) | ✓ | 4.18% |  |
| 25 | `GrossProfitYOY` | 毛利率同比 | number(18,4) | ✓ | 4.16% |  |
| 26 | `GrossProfitIncrease` | 毛利率比上年同期增减 | number(14,8) | ✓ | 13.74% | 毛利率比上年同期增减(GrossProfitIncrease)：优先取原文披露值，如未披露，则计算，计算公式=毛利率-上... |
...
日期类型(DateType)与(CT_SystemConst)表中的DM字段关联，令LB = 1074 and DM IN(2,3)，得到日期类型的具体描述：2-季度，3-期末累计。
...
分类方式(Classification)与(CT_SystemConst)表中的DM字段关联，令LB = 1046 AND DM NOT IN(50)，得到分类方式的具体描述：10-按行业，20-按产品，30-按地区，40-按缴费方式，60-按销售模式，70-按品牌，80-按渠道模式。
...
毛利率(GrossProfit)：优先取原文披露值，如未披露，则计算，计算公式=当主营业务收入>0时，（主营业务收入-主营业务成本）/ABS主营业务收入，否则不计算.

### GrossProfitFormerYear (上年同期毛利率)

上年同期毛利率(GrossProfitFormerYear)：优先取原文披露值，如未披露，则计算，计算公式=当上年同期主营业务收入>0时，（上年同期主营业务收入-上年同期主营业务成本）/ABS上年同期主营业务收入，否则不计算.
