...
| 3 | `InitialInfoPublDate` | 首次信息发布日期 | date | ✓ | 99.84% |  |
| 4 | `InfoPublDate` | 信息发布日期 | date | ✗ | 100.0% |  |
| 5 | `InfoSource` | 信息来源 | varchar2(50) | ✓ | 99.84% |  |
| 6 | `AnnouncementType` | 公告类型 | number(10) | ✓ | 100.0% | 公告类型(AnnouncementType)与(CT_SystemConst)表中的DM字段关联，令LB = 1109，... |
| 7 | `DisclosureMethod` | 披露方式 | number(10) | ✓ | 99.84% | 披露方式(DisclosureMethod)与(CT_SystemConst)表中的DM字段关联，令LB = 1221，... |
| 8 | `EventContent` | 事件内容 | clob | ✓ | 0.51% |  |
| 9 | `ActionDesc` | 行为描述 | varchar2(200) | ✓ | 99.84% |  |
| 10 | `NewestAdvance` | 最新进展状态描述 | varchar2(200) | ✓ | 1.53% |  |
| 11 | `EventSubject` | 事件主体 | number(10) | ✓ | 100.0% | 事件主体(EventSubject)与(CT_SystemConst)表中的DM字段关联，令LB = 1246，得到事件... |
| 12 | `EventProcedure` | 事件进程 | number(10) | ✓ | 99.84% | 事件进程(EventProcedure)与(CT_SystemConst)表中的DM字段关联，令LB = 1059，得到... |
| 13 | `ActionWays` | 行为方式 | number(10) | ✓ | 99.84% | 行为方式(ActionWays)与(CT_SystemConst)表中的DM字段关联，令LB = 1063，得到行为方式... |
| 14 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 99.84% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币... |
| 15 | `SubjectName` | 事件主体名称 | varchar2(100) | ✓ | 99.84% |  |
| 16 | `SubjectCode` | 事件主体企业编号 | number(10) | ✓ | 1.02% | 事件主体企业编号(SubjectCode)和机构基本资料表(CompanyCode)关联 |
| 17 | `SubjectAssociation` | 与上市公司关联关系 | number(10) | ✓ | 99.84% | 与上市公司关联关系(SubjectAssociation)与(CT_SystemConst)表中的DM字段关联，令LB ... |
| 18 | `ObjectName` | 交易对象名称 | varchar2(100) | ✓ | 99.84% |  |
| 19 | `ObjectCode` | 交易对象企业编号 | number(10) | ✓ | 0.05% | 交易对象企业编号(ObjectCode)和机构基本资料表(CompanyCode)关联 |
| 20 | `ObjectAssociation` | 与上市公司关联关系 | number(10) | ✓ | 99.84% | 与上市公司关联关系(ObjectAssociation)与(CT_SystemConst)表中的DM字段关联，令LB =... |
| 21 | `AgreementDate` | 协议签署日期 | date | ✓ | 0.19% |  |
| 22 | `IfEnded` | 是否终止 | number(10) | ✓ | 0.24% | 是否终止（IfEnded），该字段固定以下常量：0-否；1-是 |
| 23 | `Note` | 备注 | varchar2(300) | ✓ | 0.01% |  |
| 24 | `SubsidyTaxType` | 税负类别 | number(10) | ✓ |  |  |
| 25 | `SubsidySum` | 补贴总金额(元) | number(19,4) | ✓ | 99.9% |  |
| 26 | `ApportionTerm` | 分摊年限(年) | number(18,4) | ✓ | 0.0% |  |
| 27 | `SubsidyBeginDate` | 补贴实现起始日 | date | ✓ | 0.0% |  |
| 28 | `SunsidyEndDate` | 补贴实现截止日 | date | ✓ | 0.0% |  |
| 29 | `LatestReceivedSubsidySum` | 本次收到补贴金额(元) | number(19,4) | ✓ | 98.27% |  |
| 30 | `TotalReceivedSubsidySum` | 累计收到补贴金额(元) | number(19,4) | ✓ | 98.27% |  |
| 31 | `XGRQ` | 修改日期 | date | ✗ |  |  |
