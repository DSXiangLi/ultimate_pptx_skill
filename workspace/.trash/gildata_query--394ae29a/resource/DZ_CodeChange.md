...
1、内容说明：收录证券代码的历次变更情况，包括证券市场、代码定义、启用日期和停用日期等内容。
2、数据范围：A股上市公司
3、信息来源：上交所
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `SecuCode` | 证券代码 | varchar2(20) | ✗ | 100.0% |  |
| 3 | `SecuAbbr` | 证券简称 | varchar2(100) | ✓ | 3.01% | 证券简称(SecuAbbr)：此字段仅当代码定义(CodeDefine)=16-上交所质押式回购申报代码时有数据 |
| 4 | `SecuMarket` | 证券市场 | number(10) | ✓ | 95.03% | 证券市场(SecuMarket)与(CT_SystemConst)表中的DM字段关联，令LB = 201，得到证券市场的... |
| 5 | `CodeDefine` | 代码定义 | number(10) | ✓ | 100.0% | 代码定义(CodeDefine)与(CT_SystemConst)表中的DM字段关联，令LB = 1006，得到代码定义... |
| 6 | `InnerCode` | 内部编码 | number(10) | ✗ | 100.0% |  |
| 7 | `CompanyCode` | 公司编码 | number(10) | ✗ | 100.0% |  |
| 8 | `BeginDate` | 启用日期 | date | ✓ | 76.52% |  |
| 9 | `StopDate` | 停用日期 | date | ✓ | 75.28% |  |
...
证券简称(SecuAbbr)：此字段仅当代码定义(CodeDefine)=16-上交所质押式回购申报代码时有数据
...
代码定义(CodeDefine)与(CT_SystemConst)表中的DM字段关联，令LB = 1006，得到代码定义的具体描述：1-交易所，2-托管银行，3-聚源数据，5-开放式基金场内申购代码，6-开放式基金二级交易代码，7-深交所揭示净值代码，8-基金披露净值代码，9-其他，10-开放式基金其他媒体代码，11-债券托管代码，12-上交所固定收益平台，13-沪市申购代码，14-沪市交易代码，15-固定收益平台唯一交易场所，16-上交所质押式回购申报代码，17-深交所唯一交易场所综合协议平台，18-深交所综合收益平台，19-上交所特定债券平台，20-深交所特定债券平台，21-上交所大宗交易，22-上交所综合业务平台定转交易。
