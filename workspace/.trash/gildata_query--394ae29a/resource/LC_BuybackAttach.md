...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `BuybackID` | 股份回购ID | number(19) | ✗ | 100.0% | 股份回购ID（BuybackID）：与LC_Buyback表的ID字段关联。 |
| 3 | `InfoPublDate` | 信息发布日期 | date | ✗ | 100.0% |  |
| 4 | `EffectiveDate` | 截止日期/实施日期 | date | ✗ | 100.0% |  |
| 5 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 100.0% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币... |
| 6 | `BuybackSum` | 本次回购数量(股)(份) | number(18,4) | ✓ | 100.0% |  |
| 7 | `Percentage` | 占总股本比例 | number(18,8) | ✓ | 100.0% |  |
| 8 | `CumulativeSum` | 累计回购数量(股)(份) | number(18,4) | ✓ | 100.0% |  |
| 9 | `CumulativeSumToTS` | 累计回购数量占总股本的比例 | number(18,8) | ✓ | 100.0% |  |
| 10 | `HighPrice` | 本次购买最高价(元/股)(元/份) | number(18,4) | ✓ | 74.59% |  |
| 11 | `LowPrice` | 本次购买最低价(元/股)(元/份) | number(18,4) | ✓ | 77.84% |  |
| 12 | `BuybackFunds` | 本次回购资金(元) | number(19,4) | ✓ | 96.89% |  |
| 13 | `CumulativeValueSum` | 累计回购资金总额(元) | number(19,4) | ✓ | 96.84% |  |
...
股份回购ID（BuybackID）：与LC_Buyback表的ID字段关联。
