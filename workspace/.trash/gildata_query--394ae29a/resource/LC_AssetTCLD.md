...
1.公司资产托管承包租赁及赠与等重大事项，包括事件主体/交易对象名称、企业编号、与上市公司关联关系，涉及金额等指标
2.数据范围：2001-至今
3.信息来源：上市公司公告
...
| 3 | `InitialInfoPublDate` | 首次信息发布日期 | date | ✓ | 95.66% |  |
| 4 | `InfoPublDate` | 信息发布日期 | date | ✗ | 100.0% |  |
| 5 | `InfoSource` | 信息来源 | varchar2(50) | ✓ | 95.66% |  |
| 6 | `AnnouncementType` | 公告类型 | number(10) | ✓ | 100.0% | 公告类型(AnnouncementType)与(CT_SystemConst)表中的DM字段关联，令LB = 1109，... |
| 7 | `DisclosureMethod` | 披露方式 | number(10) | ✓ | 95.66% | 披露方式(DisclosureMethod)与(CT_SystemConst)表中的DM字段关联，令LB = 1221，... |
| 8 | `EventContent` | 事件内容 | clob | ✓ | 2.16% |  |
| 9 | `ActionDesc` | 行为描述 | varchar2(200) | ✓ | 95.66% |  |
| 10 | `NewestAdvance` | 最新进展状态描述 | varchar2(200) | ✓ | 25.5% |  |
| 11 | `EventSubject` | 事件主体 | number(10) | ✓ | 99.87% | 事件主体(EventSubject)与(CT_SystemConst)表中的DM字段关联，令LB = 1246，得到事件... |
| 12 | `EventProcedure` | 事件进程 | number(10) | ✓ | 95.66% | 事件进程(EventProcedure)与(CT_SystemConst)表中的DM字段关联，令LB = 1059，得到... |
| 13 | `ActionWays` | 行为方式 | number(10) | ✓ | 95.66% | 行为方式(ActionWays)与(CT_SystemConst)表中的DM字段关联，令LB = 1063，得到行为方式... |
| 14 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 95.66% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币... |
| 15 | `SubjectName` | 事件主体名称 | varchar2(100) | ✓ | 95.66% |  |
| 16 | `SubjectCode` | 事件主体企业编号 | number(10) | ✓ | 3.41% | 事件主体企业编号(SubjectCode)和机构基本资料表(CompanyCode)关联 |
| 17 | `SubjectAssociation` | 与上市公司关联关系 | number(10) | ✓ | 95.66% | 与上市公司关联关系(SubjectAssociation)与(CT_SystemConst)表中的DM字段关联，令LB ... |
| 18 | `ObjectName` | 交易对象名称 | varchar2(100) | ✓ | 94.42% |  |
| 19 | `ObjectCode` | 交易对象企业编号 | number(10) | ✓ | 3.25% | 交易对象企业编号(ObjectCode)和机构基本资料表(CompanyCode)关联 |
| 20 | `ObjectAssociation` | 与上市公司关联关系 | number(10) | ✓ | 94.42% | 与上市公司关联关系(ObjectAssociation)与(CT_SystemConst)表中的DM字段关联，令LB =... |
| 21 | `AgreementDate` | 协议签署日期 | date | ✓ | 7.19% |  |
| 22 | `IfEnded` | 是否终止 | number(10) | ✓ | 6.91% | 是否终止(IfEnded)：该字段固定以下常量：0->否1->是 |
| 23 | `Note` | 备注 | varchar2(300) | ✓ | 0.53% |  |
| 24 | `EventType` | 事项类型 | number(10) | ✗ | 100.0% | 事项类型(EventType)与(CT_SystemConst)表中的DM字段关联，令LB = 1062，得到事项类型的... |
| 25 | `AssetCustodianSum` | 涉及金额(元) | number(19,4) | ✓ | 40.1% |  |
| 26 | `BestowalSum` | 赠与金额(元) | number(19,4) | ✓ | 14.5% |  |
| 27 | `XGRQ` | 修改日期 | date | ✗ |  |  |
