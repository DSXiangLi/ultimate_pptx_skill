...
| 属性 | 值 |
|------|-----|
| 表名 | `MF_FundCorrelCoeffi` |
| MySQL表名 | `mf_fundcorrelcoeffi` |
| 中文名 | 基金相关系数 |
...
1.内容说明：用于衡量基金与大盘的相关性，相关系数为正（负），则代表与沪深300指数正（负）相关，相关系数的绝对值越大，代表与沪深300指数的相关性越强
2.数据范围：2005年月1月起-至今。
3.信息来源：根据基金每个交易日公布的复权单位净值、沪深300指数收盘价计算而得。公式为：Covariance(基金收益率，沪深300涨跌幅)/{std(基金收益率)*std(沪深300涨跌幅)}，基金收益率的样本首日需大于等于截止日期往前追溯相应周期的日历日期加一天
