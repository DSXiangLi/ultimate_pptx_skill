...
1.内容说明：公司自有资金、通过发行新股、增发新股、配股等方式所得募集资金的项目投资情况以及运用进展和改投状况。
2.数据范围：1988-12-01至今
...
| 4 | `InitialInfoPunlDate` | 首次信息发布日期 | date | ✓ | 35.63% |  |
...
| 8 | `InvestProject` | 募资投向项目名称 | varchar2(200) | ✗ | 100.0% |  |
| 9 | `ProjectStatement` | 项目内容 | clob | ✓ | 1.48% |  |
| 10 | `PlannedSum` | 计划投入金额(元) | number(19,4) | ✓ | 86.96% |  |
| 11 | `ActualInvestEndDate` | 实际投入截至日期 | date | ✓ | 17.8% |  |
| 12 | `ActualSum` | 实际投入金额(元) | number(19,4) | ✓ | 17.82% |  |
| 13 | `Industry` | 投向行业 | number(10) | ✓ | 0.01% | 关联行业表【CT_Industry】的行业编码[IndustryNum]，获取对应的行业信息 |
| 14 | `InvestField` | 投向领域 | number(10) | ✓ |  |  |
| 15 | `ProceedingStatement` | 进展和收益说明 | clob | ✓ | 16.73% |  |
| 16 | `IfSwitched` | 改投与否 | number(10) | ✓ | 94.08% | 改投与否(IfSwitched)与(CT_SystemConst)表中的DM字段关联，令LB = 1219，得到改投与否... |
| 17 | `ProjectSwitchedTo` | 改投项目 | varchar2(255) | ✓ | 3.11% |  |
| 18 | `SumSwitched` | 改投金额(元) | number(19,4) | ✓ | 1.74% |  |
| 19 | `PurchaseType` | 收购兼并类型 | number(10) | ✓ | 12.45% | 收购兼并类型(PurchaseType)与(CT_SystemConst)表中的DM字段关联，令LB = 1304，得到... |
| 20 | `BookValue` | 收购资产账面价值(元) | number(19,4) | ✓ | 0.67% |  |
| 21 | `AppraisalValue` | 收购资产评估价值(元) | number(19,4) | ✓ | 1.5% |  |
| 22 | `PurchasePrice` | 收购资产价格(元) | number(19,4) | ✓ | 10.0% |  |
| 23 | `EquityRatio` | 收购权益比例(%) | number(18,6) | ✓ | 8.38% |  |
| 24 | `Transferor` | 项目出让方 | varchar2(255) | ✓ | 9.34% |  |
| 25 | `Relationship` | 与出让方关联关系 | number(10) | ✓ | 7.28% | 与出让方关联关系(Relationship)与(CT_SystemConst)表中的DM字段关联，令LB = 1036，... |
...
| 28 | `TargetName` | 收购标的名称 | varchar2(255) | ✓ | 12.34% |  |
