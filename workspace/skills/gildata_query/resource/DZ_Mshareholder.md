...
| 4 | `EndDate` | 截止日期 | date | ✓ | 70.62% |  |
| 5 | `InfoSource` | 信息来源 | varchar2(50) | ✓ | 97.89% |  |
| 6 | `BulletinType` | 公告类别 | number(10) | ✓ | 39.4% | 公告类别(BulletinType)与(CT_SystemConst)表中的DM字段关联，令LB=1311，得到公告类别... |
| 7 | `MSHName` | 股东名称 | varchar2(120) | ✗ | 100.0% |  |
| 8 | `SHAttribute` | 股东性质 | number(10) | ✓ | 100.0% | 股东性质(SHAttribute)与(CT_SystemConst)表中的DM字段关联，令LB=1783，得到股东性质的... |
| 9 | `GDID` | 股东ID | number(10) | ✓ | 72.76% | 当股东属性（SHAttribute）=2时，与机构基本资料（LC_InstiArchive）中的企业编号（Company... |
| 10 | `MSHPercentage` | 持股比例 | number(9,6) | ✓ | 82.24% |  |
| 11 | `MSHNumber` | 股东地位 | number(10) | ✗ | 100.0% | 股东地位(MSHNumber)与(CT_SystemConst)表中的DM字段关联，令LB=1198，得到股东地位的具体... |
| 12 | `GetMethod` | 股权获取方式 | number(10) | ✓ | 22.66% | 股权获取方式(GetMethod)与(CT_SystemConst)表中的DM字段关联，令LB=1200，得到股权获取方... |
| 13 | `LegalRepr` | 法人代表 | varchar2(50) | ✓ | 10.03% |  |
| 14 | `RegCapital` | 注册资本(元) | number(19,4) | ✓ | 10.63% |  |
| 15 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 10.63% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB=1068，得到货币单位... |
| 16 | `MainBusiness` | 主营业务 | varchar2(255) | ✓ | 7.97% |  |
| 17 | `EconomicNature` | 经济性质 | number(10) | ✓ |  |  |
| 18 | `NationalityDesc` | 国籍描述 | varchar2(256) | ✓ | 8.69% |  |
| 19 | `PermanentResidency` | 永久境外居留权 | varchar2(256) | ✓ | 6.04% |  |
| 20 | `BackgroundIntr` | 背景介绍 | clob | ✓ | 22.81% |  |
| 21 | `IfExisted` | 存在与否 | number(10) | ✓ | 100.0% | 该字段固定以下常量：1-有；0-无 |
| 22 | `StructureChart` | 实际控制人结构图 | blob | ✓ | 0.35% |  |
| 23 | `FileType` | 文件格式 | number(10) | ✓ | 5.87% | 文件格式(FileType)与(CT_SystemConst)表中的DM字段关联，令LB=1309，得到文件格式的具体描... |
| 24 | `XGRQ` | 更新时间 | date | ✗ |  |  |
