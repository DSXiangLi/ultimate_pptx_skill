...
| 3 | `ContractCode` | 合约代码 | varchar2(20) | ✗ | 100.0% |  |
| 4 | `ContractAbbr` | 合约简称 | varchar2(50) | ✗ | 100.0% |  |
| 5 | `ContractName` | 合约全称 | varchar2(100) | ✓ | 100.0% |  |
| 6 | `ExchangeCode` | 交易所代码 | number(10) | ✗ | 100.0% | 交易所代码(ExchangeCode)与(CT_SystemConst)表中的DM字段关联，令LB = 201，得到交易... |
| 7 | `OptionCode` | 合约标的 | varchar2(100) | ✗ | 100.0% |  |
| 8 | `Quotations` | 报价方式 | varchar2(50) | ✓ | 100.0% |  |
| 9 | `PriceUnit` | 单位报价量 | varchar2(50) | ✓ | 100.0% |  |
| 10 | `LittlestChangeUnit` | 单位变动点 | varchar2(50) | ✓ | 100.0% |  |
| 11 | `ChangePCTLimit` | 每日涨跌幅度 | varchar2(200) | ✓ | 100.0% |  |
| 12 | `CurrencyCode` | 货币单位 | number(10) | ✓ | 100.0% | 货币单位(CurrencyCode)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币... |
| 13 | `DeliveryYear` | 交割年 | varchar2(4) | ✓ | 100.0% |  |
| 14 | `DeliveryMonth` | 交割月 | varchar2(2) | ✓ | 100.0% |  |
| 15 | `EffectiveDate` | 新合约上市日 | date | ✓ | 92.12% |  |
| 16 | `LastTradingDate` | 最后交易日 | date | ✓ | 100.0% |  |
| 17 | `DeliveryDate` | 交割日期 | date | ✓ | 0.0% |  |
| 18 | `LastDeliveryDate` | 最后交割日期 | date | ✓ | 100.0% |  |
| 19 | `DeliveryMethod` | 交割方式 | number(10) | ✓ | 100.0% | 交割方式(DeliveryMethod)与(CT_SystemConst)表中的DM字段关联，令LB = 1870，得到... |
| 20 | `MinMarginRatio` | 最低交易保证金率(%) | number(9,6) | ✓ | 100.0% |  |
| 21 | `TradingCommission` | 交易手续费 | varchar2(100) | ✓ | 0.0% |  |
| 22 | `DeliveryCommission` | 交割手续费 | varchar2(100) | ✓ | 0.0% |  |
| 23 | `SettPriceDesc` | 结算价计算方式 | clob | ✓ | 100.0% |  |
| 24 | `DeliSettPriceDesc` | 交割结算价计算方式 | clob | ✓ | 100.0% |  |
| 25 | `ListBasisPrice` | 挂牌基准价 | number(19,4) | ✓ | 91.13% |  |
| 26 | `ChangePCTListPrice` | 挂牌价涨跌幅(%) | number(18,10) | ✓ | 36.95% |  |
| 27 | `ContractState` | 合约状态 | number(10) | ✓ | 100.0% | 合约状态(ContractState)与(CT_SystemConst)表中的DM字段关联，令LB = 1790，得到合... |
| 28 | `ChangeReason` | 变动原因 | varchar2(200) | ✓ | 0.0% |  |
| 29 | `ContractMonth` | 合约月份 | varchar2(100) | ✓ | 100.0% |  |
| 30 | `ExchangeDate` | 交易时间 | varchar2(200) | ✓ | 100.0% |  |
| 31 | `LastTradingDateDesc` | 最后交易日描述 | varchar2(100) | ✓ | 72.41% |  |
| 32 | `LastTradingTimeDesc` | 最后交易日交易时间描述 | varchar2(100) | ✓ | 100.0% |  |
| 33 | `DeliveryDateDesc` | 交割日期描述 | varchar2(100) | ✓ | 0.0% |  |
| 34 | `LastDeliveryDateDesc` | 最后交割日日期描述 | varchar2(100) | ✓ | 100.0% |  |
| 35 | `ContractIntroduction` | 合约简介 | clob | ✓ | 33.5% |  |
...
交割方式(DeliveryMethod)与(CT_SystemConst)表中的DM字段关联，令LB = 1870，得到交割方式的具体描述：1-现金交割，2-实物交割。

### ContractState (合约状态)

合约状态(ContractState)与(CT_SystemConst)表中的DM字段关联，令LB = 1790，得到合约状态的具体描述：1-正常，2-终止，3-暂停，99-其他。
