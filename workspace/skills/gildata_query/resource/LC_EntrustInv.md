...
1.公司委托贷款等重大事项，包括事件主体/交易对象名称、企业编号、与上市公司关联关系、涉及金额、委托期限、委托起始日、委托截止日等指标。
2.数据范围：2001-至今
3.信息来源：上市公司公告
...
| 3 | `InitialInfoPublDate` | 首次信息发布日期 | date | ✓ | 85.63% |  |
| 4 | `InfoPublDate` | 信息发布日期 | date | ✗ | 100.0% |  |
| 5 | `InfoSource` | 信息来源 | varchar2(50) | ✓ | 85.63% |  |
| 6 | `AnnouncementType` | 公告类型 | number(10) | ✓ | 100.0% | 公告类型(AnnouncementType)与(CT_SystemConst)表中的DM字段关联，令LB = 1109，... |
| 7 | `DisclosureMethod` | 披露方式 | number(10) | ✓ | 85.63% | 披露方式(DisclosureMethod)与(CT_SystemConst)表中的DM字段关联，令LB = 1221，... |
| 8 | `EventContent` | 事件内容 | clob | ✓ | 4.72% |  |
| 9 | `ActionDesc` | 行为描述 | varchar2(200) | ✓ | 85.63% |  |
| 10 | `NewestAdvance` | 最新进展状态描述 | varchar2(200) | ✓ | 31.0% |  |
| 11 | `EventSubject` | 事件主体 | number(10) | ✓ | 100.0% | 事件主体(EventSubject)与(CT_SystemConst)表中的DM字段关联，令LB = 1246，得到事件... |
| 12 | `EventProcedure` | 事件进程 | number(10) | ✓ | 85.63% | 事件进程(EventProcedure)与(CT_SystemConst)表中的DM字段关联，令LB = 1059，得到... |
| 13 | `ActionWays` | 行为方式 | number(10) | ✓ | 85.63% | 行为方式(ActionWays)与(CT_SystemConst)表中的DM字段关联，令LB = 1063，得到行为方式... |
| 14 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 85.63% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币... |
| 15 | `SubjectName` | 事件主体名称 | varchar2(100) | ✓ | 85.63% |  |
| 16 | `SubjectCode` | 事件主体企业编号 | number(10) | ✓ | 0.63% | 事件主体企业编号(SubjectCode)：与“机构基本资料表(LC_InstiArchive)”中的“公司代码(Com... |
| 17 | `SubjectAssociation` | 与上市公司关联关系 | number(10) | ✓ | 85.63% | 与上市公司关联关系(SubjectAssociation)与(CT_SystemConst)表中的DM字段关联，令LB ... |
| 18 | `ObjectName` | 交易对象名称 | varchar2(100) | ✓ | 85.63% |  |
| 19 | `ObjectCode` | 交易对象企业编号 | number(10) | ✓ | 1.14% | 交易对象企业编号(ObjectCode)：与“机构基本资料表(LC_InstiArchive)”中的“公司代码(Comp... |
| 20 | `ObjectAssociation` | 与上市公司关联关系 | number(10) | ✓ | 85.63% | 与上市公司关联关系(ObjectAssociation)与(CT_SystemConst)表中的DM字段关联，令LB =... |
| 21 | `AgreementDate` | 协议签署日期 | date | ✓ | 3.97% |  |
| 22 | `IfEnded` | 是否终止 | number(10) | ✓ | 4.52% | 是否终止(IfEnded)，该字段固定以下常量：0-否；1-是 |
| 23 | `Note` | 备注 | varchar2(300) | ✓ | 0.21% |  |
| 24 | `EntrustFinanceSum` | 涉及金额(元) | number(19,4) | ✓ | 99.96% |  |
| 25 | `EntrustFinanceTerm` | 委托期限(月) | number(18,4) | ✓ | 72.07% |  |
| 26 | `EntrustFinanceBeginDate` | 委托起始日 | date | ✓ | 15.62% |  |
| 27 | `EntrustFinanceEndDate` | 委托截止日 | date | ✓ | 18.14% |  |
| 28 | `PromisedIncome` | 约定收益说明 | varchar2(255) | ✓ | 53.07% |  |
| 29 | `ActualIncome` | 回收时实现收益(元) | number(19,4) | ✓ | 8.86% |  |
| 30 | `XGRQ` | 修改日期 | date | ✗ |  |  |
