...
1.内容说明：本表主要统计分析师和研究机构对港股市场股票盈利预测指标的研究状况
2.数据范围：2020年至今数据
3.信息来源：Factset
...
| 4 | `IndicatorType` | 预测指标类型 | number(10) | ✗ | 100.0% | 预测指标类型(IndicatorType)与(CT_SystemConst)表中的DM字段关联，令LB = 2260 a... |
| 5 | `ForecastYearType` | 预测报告期类型 | number(10) | ✗ | 100.0% | 预测报告期类型(ForecastYearType)与(CT_SystemConst)表中的DM字段关联，令LB = 20... |
| 6 | `ForecastYear` | 预测报告期 | date | ✗ | 100.0% |  |
| 7 | `StatPeriod` | 统计周期(天) | number(10) | ✓ | 100.0% | 统计周期(天)（StatPeriod）：该字段固定常量：100-100天 |
| 8 | `CurrencyCode` | 预测数据币种 | number(10) | ✓ | 100.0% | 预测数据币种(CurrencyCode)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到... |
| 9 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 100.0% | 货币单位(CurrencyUnit)：该字段表示预测值数值单位，固定以下常量：1-万(乘10000)、2-原始(乘1)、... |
| 10 | `TimeSpan` | 预测时间跨度 | number(10) | ✓ | 100.0% | 预测时间跨度（TimeSpan）：预测最近一次定报，为1，预测下一次定报，为2，随时间动态更新 |
| 11 | `ForcastValueMean` | 预测值算数平均数 | number(19,6) | ✓ | 100.0% |  |
| 12 | `ForcastValueMed` | 预测值中位数 | number(19,6) | ✓ | 100.0% |  |
| 13 | `ForcastNum` | 预测机构数 | number(10) | ✓ | 100.0% |  |
| 14 | `ForcastValueMin` | 预测值最小值 | number(19,6) | ✓ | 100.0% |  |
| 15 | `ForcastValueMax` | 预测值最大值 | number(19,6) | ✓ | 100.0% |  |
| 16 | `ForcastValueStdDev` | 预测值离散程度 | number(19,6) | ✓ | 58.25% |  |
| 17 | `ForcastValueUpNum` | 预测值向上修正数量 | number(10) | ✓ | 100.0% |  |
| 18 | `ForcastValueDownNum` | 预测值向下修正数量 | number(10) | ✓ | 100.0% |  |
...
### IndicatorType (预测指标类型)

预测指标类型(IndicatorType)与(CT_SystemConst)表中的DM字段关联，令LB = 2260 and DM in (1,2,3,4,5,6,7,8)，得到预测指标类型的具体描述：1-每股现金流，2-每股红利，3-每股收益，4-合并每股收益，5-母公司每股收益，6-营业收入，7-合并营业收入，8-母公司营业收入。

### ForecastYearType (预测报告期类型)

预测报告期类型(ForecastYearType)与(CT_SystemConst)表中的DM字段关联，令LB = 2075 and DM in (4,5,6)，得到预测报告期类型的具体描述：4-季报，5-半年报，6-年报。
