# MF_REITsCSIValuation

**中文名**: 中证基础设施基金(REITs)估值

## 基本信息

| 属性 | 值 |
|------|-----|
| 表名 | `MF_REITsCSIValuation` |
| MySQL表名 | `mf_reitscsivaluation` |
| 中文名 | 中证基础设施基金(REITs)估值 |
...
本表用于记录中证发布的REITs估值信息，包括估值价格及其他估值参数。
...
| 4 | `SecuMarket` | 证券市场 | number(10) | ✓ | 100.0% | 证券市场(SecuMarket)与(CT_SystemConst)表中的DM字段关联，令LB=201，得到证券市场的具体... |
| 5 | `ValuePrice` | REITs估值价格(元/份) | number(18,8) | ✓ | 100.0% |  |
| 6 | `ABSValuePrice` | ABS估值(元/份) | number(18,8) | ✓ | 100.0% |  |
| 7 | `ABSYield` | ABS估值收益率(%) | number(18,8) | ✓ | 100.0% |  |
| 8 | `PVRatio` | PV比值 | number(18,8) | ✓ | 100.0% |  |
| 9 | `YrMat` | 剩余期限(年) | number(18,8) | ✓ | 100.0% |  |
...
```sql
-- 查询 中证基础设施基金(REITs)估值 数据
SELECT *
FROM mf_reitscsivaluation
