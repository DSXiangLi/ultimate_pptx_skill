...
1.收录B股首次发行上市的明细情况，包括发行概况、配售日期、承配缴款日、承销日期、发行结果等内容。
2.数据范围：1991-10-31至今
...
| 4 | `IntentLetterPublDate` | 招股意向书发布日期 | date | ✓ | 0.0% |  |
| 5 | `ProspectusPublDate` | 招股说明书发布日期 | date | ✓ | 100.0% |  |
| 6 | `ListAnnouncementDate` | 上市公告书发布日期 | date | ✓ | 97.37% |  |
| 7 | `RaisingMethod` | 募资方式 | number(10) | ✓ | 100.0% | 募资方式(RaisingMethod)与(CT_SystemConst)表中的DM字段关联，令LB = 1021 AND... |
| 8 | `StockType` | 发行股票类型 | number(10) | ✓ | 100.0% | 发行股票类型(StockType)与(CT_SystemConst)表中的DM字段关联，令LB = 1177，得到发行股... |
| 9 | `ParValueRMB` | 每股面值(人民币元) | number(19,4) | ✓ | 100.0% |  |
| 10 | `PriceUnit` | 计价单位 | number(10) | ✓ | 100.0% | 计价单位(PriceUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到计价单位的... |
| 11 | `IssuePriceRMB` | 每股发行价(人民币元) | number(19,4) | ✓ | 100.0% |  |
| 12 | `IssuePriceFC` | 每股发行价(外币元) | number(19,4) | ✓ | 100.0% |  |
| 13 | `WeightedPERatio` | 加权平均发行市盈率(倍) | float(23) | ✓ | 35.96% |  |
| 14 | `DilutedPERatio` | 全面摊薄发行市盈率(倍) | float(23) | ✓ | 52.63% |  |
| 15 | `StraInvestorPlaPriceFC` | 战略投资者配售价(外币元) | number(19,4) | ✓ | 3.51% |  |
| 16 | `IssueVolPlanned` | 计划发行量(股) | number(16,0) | ✓ | 100.0% |  |
| 17 | `OverAllotmentOption` | 超额配售权(股) | number(16,0) | ✓ | 18.42% |  |
| 18 | `IssueMethod` | 发行方式 | number(10) | ✓ |  |  |
| 19 | `IssueObject` | 发行对象 | varchar2(255) | ✓ | 100.0% |  |
| 20 | `PlaStartDate` | 配售起始日 | date | ✓ | 100.0% |  |
| 21 | `PlaEndDate` | 配售截止日 | date | ✓ | 100.0% |  |
| 22 | `PayStartDate` | 承配缴款起始日 | date | ✓ | 5.26% |  |
| 23 | `PayEndDate` | 承配缴款截止日 | date | ✓ | 6.14% |  |
| 24 | `UnderwritingStartDate` | 承销起始日 | date | ✓ | 33.33% |  |
| 25 | `UnderwritingEndDate` | 承销截止日 | date | ✓ | 34.21% |  |
| 26 | `PreparedListExchange` | 拟上市地 | number(10) | ✓ | 100.0% | 拟上市地(PreparedListExchange)与(CT_SystemConst)表中的DM字段关联，令LB = 2... |
| 27 | `DividendPolicy` | 股利分配政策 | varchar2(100) | ✓ | 35.96% |  |
| 28 | `EstimatedFirstDiviDate` | 预计首次分配时间 | varchar2(50) | ✓ | 3.51% |  |
| 29 | `UnderwritingMode` | 承销方式 | number(10) | ✓ | 100.0% | 承销方式(UnderwritingMode)与(CT_SystemConst)表中的DM字段关联，令LB = 1017，... |
| 30 | `ActualIssueVol` | 实际发行量(股) | number(16,0) | ✓ | 100.0% |  |
| 31 | `TotalIssueMVRMB` | 发行总市值(人民币元) | number(19,4) | ✓ | 100.0% |  |
| 32 | `IssueCostRMB` | 发行费用总额(人民币元) | number(19,4) | ✓ | 42.98% |  |
| 33 | `IPOProceedsRMB` | 募集资金总额(人民币元) | number(19,4) | ✓ | 100.0% |  |
| 34 | `IPONetProceedsRMB` | 募集资金净额(人民币元) | number(19,4) | ✓ | 43.86% |  |
| 35 | `IssueCostPerShareRMB` | 每股发行费用(人民币元) | number(19,4) | ✓ | 18.42% |  |
| 36 | `TotalIssueMVFC` | 发行总市值(外币元) | number(19,4) | ✓ | 100.0% |  |
| 37 | `IssueCostFC` | 发行费用总额(外币元) | number(19,4) | ✓ | 34.21% |  |
| 38 | `IPOProceedsFC` | 募集资金总额(外币元) | number(19,4) | ✓ | 100.0% |  |
| 39 | `IPONetProceedsFC` | 募集资金净额(外币元) | number(19,4) | ✓ | 35.96% |  |
| 40 | `IssueCostPerShareFC` | 每股发行费用(外币元) | number(19,4) | ✓ | 11.4% |  |
| 41 | `DateToAccount` | 募集资金到帐时间 | date | ✓ | 0.88% |  |
| 42 | `ListDate` | 上市日期 | date | ✓ | 100.0% |  |
| 43 | `OutstandingShares` | 本次上市流通股数(股) | number(16,0) | ✓ | 99.12% |  |
| 44 | `StraInvestorPlaVol` | 战略投资者配售股数(股) | number(16,0) | ✓ | 6.14% |  |
| 45 | `PlaNum` | 配售户数(户) | number(10) | ✓ | 36.84% |  |
| 46 | `UnderwriterBoughtVol` | 余股包销数量(股) | number(16,0) | ✓ | 0.88% |  |
| 47 | `FirstOpenPrice` | 上市首日开盘价(外币元) | number(19,4) | ✓ | 0.0% |  |
| 48 | `FirstClosePrice` | 上市首日收盘价(外币元) | number(19,4) | ✓ | 0.0% |  |
| 49 | `FirstTurnover` | 上市首日换手率 | number(9,6) | ✓ | 0.0% |  |
| 50 | `EarningForecastYear` | 盈利预测年度 | date | ✓ | 0.0% |  |
| 51 | `MainIncomeForecast` | 主营业务收入预测(人民币元) | number(19,4) | ✓ | 11.4% |  |
| 52 | `NetProfitForecast` | 净利润预测(人民币元) | number(19,4) | ✓ | 47.37% |  |
| 53 | `DilutedEPSForecast` | 全面摊薄每股盈利预测(人民币元) | number(19,4) | ✓ | 42.11% |  |
| 54 | `XGRQ` | 更新时间 | date | ✗ |  |  |
...
### PreparedListExchange (拟上市地)

拟上市地(PreparedListExchange)与(CT_SystemConst)表中的DM字段关联，令LB = 201 AND DM IN (83,90)，得到拟上市地的具体描述：83-上海证券交易所，90-深圳证券交易所。
