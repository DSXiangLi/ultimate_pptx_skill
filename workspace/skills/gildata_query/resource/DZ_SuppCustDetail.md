...
| 属性 | 值 |
|------|-----|
| 表名 | `DZ_SuppCustDetail` |
| MySQL表名 | `dz_suppcustdetail` |
| 中文名 | 公司供应商与客户 |
...
| 7 | `RelationType` | 关系类型 | number(10) | ✗ | 100.0% | 关系类型(RelationType)与(CT_SystemConst)表中的DM字段关联，令LB=1590 AND DM... |
...
| 10 | `RelatedPartyCode` | 供应商/客户代码 | number(10) | ✓ | 23.47% | 供应商/客户代码(RelatedPartyCode)：与“机构基本资料（LC_InstiArchive）”中的“企业编号... |
| 11 | `RelatedPartyAttribute` | 供应商/客户属性 | number(10) | ✓ | 100.0% | 供应商/客户属性(RelatedPartyAttribute)与(CT_SystemConst)表中的DM字段关联，令L... |
| 12 | `TargetName` | 交易标的名称 | varchar2(100) | ✓ | 17.56% |  |
| 13 | `TargetCode` | 交易标的代码 | number(10) | ✓ | 8.14% | 交易标的代码(TargetCode)：与“科创板产品主表（LC_STIBProductsMain）”中的“产品内部编码（... |
| 14 | `TradingValue` | 交易金额(元) | number(19,4) | ✓ | 97.29% |  |
| 15 | `Ratio` | 占比(%) | number(9,6) | ✓ | 99.29% |  |
| 16 | `Remark` | 备注 | varchar2(1000) | ✓ | 12.17% |  |
...
交易标的代码(TargetCode)：与“科创板产品主表（LC_STIBProductsMain）”中的“产品内部编码（InnerCode）”关联，得到交易标的的产品名称。
