...
1.本表存放公司重大经营合同的事项，包括事件主体/交易对象名称、企业编号、与上市公司关联关系、合同标的、合同获得方式、涉及金额、合同起始日、合同截止日、合同期限等指标。
2.数据范围：2012-至今
3.信息来源：上市公司公告
...
| 7 | `EventContent` | 事件内容 | clob | ✓ | 1.53% |  |
| 8 | `ActionDesc` | 行为描述 | varchar2(200) | ✓ | 100.0% |  |
| 9 | `NewestAdvance` | 最新进展状态描述(摘要信息) | varchar2(200) | ✓ | 10.47% |  |
| 10 | `EventSubject` | 事件主体 | number(10) | ✓ | 100.0% | 事件主体(EventSubject)与(CT_SystemConst)表中的DM字段关联，令LB = 1246，得到事件... |
| 11 | `EventProcedure` | 事件进程 | number(10) | ✓ | 100.0% | 事件进程(EventProcedure)与(CT_SystemConst)表中的DM字段关联，令LB = 1059，得到... |
| 12 | `ActionWays` | 行为方式 | number(10) | ✓ | 100.0% | 行为方式(ActionWays)与(CT_SystemConst)表中的DM字段关联，令LB = 1063，得到行为方式... |
| 13 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 100.0% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币... |
| 14 | `SubjectName` | 事件主体名称 | varchar2(100) | ✓ | 100.0% |  |
| 15 | `SubjectCode` | 事件主体企业编号 | number(10) | ✓ | 0.04% | 事件主体企业编号（SubjectCode）：该字段属于备用字段，目前未维护 |
...
| 18 | `ObjectCode` | 交易对象企业编号 | number(10) | ✓ | 0.07% | 交易对象企业编号（ObjectCode）：该字段属于备用字段，目前未维护 |
| 19 | `ObjectAssociation` | 交易对象与上市公司关联关系 | number(10) | ✓ | 100.0% | 交易对象与上市公司关联关系(ObjectAssociation)与(CT_SystemConst)表中的DM字段关联，令... |
| 20 | `AgreementDate` | 协议签署日期 | date | ✓ | 7.8% |  |
| 21 | `ContractObject` | 合同标的 | varchar2(500) | ✓ | 99.56% |  |
| 22 | `ContractWay` | 合同获得方式 | number(10) | ✓ | 99.97% | 合同获得方式(ContractWay)与(CT_SystemConst)表中的DM字段关联，令LB = 1750，得到合... |
| 23 | `AcceptanceDate` | 中标日期 | date | ✓ | 5.71% |  |
| 24 | `AmountInvolved` | 涉及金额(元) | number(19,4) | ✓ | 90.32% |  |
| 25 | `ContractBeginDate` | 合同起始日 | date | ✓ | 2.34% |  |
| 26 | `ContractEndDate` | 合同截止日 | date | ✓ | 2.47% |  |
| 27 | `ContractPeriod` | 合同期限(月) | number(9,4) | ✓ | 4.86% |  |
| 28 | `ContractEffect` | 合同对公司的影响 | clob | ✓ | 36.63% |  |
| 29 | `RemarkDesc` | 备注 | varchar2(300) | ✓ | 0.19% |  |
...
事件主体企业编号（SubjectCode）：该字段属于备用字段，目前未维护
...
交易对象企业编号（ObjectCode）：该字段属于备用字段，目前未维护
