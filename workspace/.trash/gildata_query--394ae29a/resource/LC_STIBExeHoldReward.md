# LC_STIBExeHoldReward

**中文名**: 科创板报告期高管及核心技术人员持股薪酬

## 基本信息

| 属性 | 值 |
|------|-----|
| 表名 | `LC_STIBExeHoldReward` |
| MySQL表名 | `lc_stibexeholdreward` |
| 中文名 | 科创板报告期高管及核心技术人员持股薪酬 |
...
1.内容说明：记录科创板公司董事会、监事会、高管人员及核心技术人员在报告期的持股状况及其获取的年度报酬情况等。
2.数据范围：2019年-至今。
3.信息来源：招股说明书、定期报告等。
...
| 8 | `PersonalCode` | 人员编码 | number(19) | ✓ | 42.26% |  |
| 9 | `LeaderName` | 领导姓名 | varchar2(50) | ✗ | 100.0% |  |
| 10 | `PositionDescription` | 职位描述 | varchar2(100) | ✓ | 100.0% |  |
| 11 | `ShareDirAmountBegin` | 期初直接持股数(股/份) | number(19,0) | ✓ | 47.1% |  |
| 12 | `ShareAmount` | 期末直接间接持股合计数(股/份) | number(19,0) | ✓ | 100.0% |  |
| 13 | `ShareDirAmount` | 其中:期末直接持股数(股/份) | number(19,0) | ✓ | 89.92% |  |
| 14 | `IndirectShares` | ###期末间接持股数(股/份) | number(19,0) | ✓ | 10.1% |  |
| 15 | `HoldAShareSum` | 持有A股数量(股/份) | number(19,0) | ✓ | 89.9% |  |
| 16 | `HoldBShareSum` | 持有B股数量(股/份) | number(19,0) | ✓ | 0.0% |  |
| 17 | `HoldHShareSum` | 持有H股数量(股/份) | number(19,0) | ✓ | 0.04% |  |
| 18 | `HoldSShareSum` | 持有S股数量(股/份) | number(19,0) | ✓ | 0.0% |  |
| 19 | `HoldNShareSum` | 持有N股数量(股/份) | number(19,0) | ✓ | 0.0% |  |
| 20 | `HoldDShareSum` | 持有D股数量(股/份) | number(19,0) | ✓ | 0.0% |  |
| 21 | `HoldOthterShareSum` | 持有其他股数量(股/份) | number(19,0) | ✓ | 0.0% |  |
| 22 | `ChangeReason` | 变动原因说明 | varchar2(200) | ✓ | 10.49% |  |
| 23 | `ChangeType` | 变动原因类别 | number(10) | ✓ | 10.29% | 变动原因类别(ChangeType)与(CT_SystemConst)表中的DM字段关联，令LB=1500 AND DM... |
| 24 | `ChangeMethod` | 变动方式 | number(10) | ✓ | 10.29% | 变动方式(ChangeMethod)与(CT_SystemConst)表中的DM字段关联，令LB=1500 AND DM... |
| 25 | `TotalYearPay` | 报酬总额(元) | number(19,3) | ✓ | 65.33% |  |
| 26 | `Subsidy` | 其中:补贴津贴(元) | number(19,3) | ✓ | 1.19% |  |
| 27 | `IfOtherPay` | 是否在股东或关联单位领取报酬、津贴 | number(10) | ✓ | 55.14% | 是否在股东或关联单位领取报酬、津贴(IfOtherPay)与(CT_SystemConst)表中的DM字段关联，令LB=... |
| 28 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 70.0% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB=1068，得到货币单位... |
...
```sql
-- 查询 科创板报告期高管及核心技术人员持股薪酬 数据
SELECT *
FROM lc_stibexeholdreward
LIMIT 10;
```
