...
| 属性 | 值 |
|------|-----|
| 表名 | `MF_FundReturnRank` |
| MySQL表名 | `mf_fundreturnrank` |
| 中文名 | 公募基金收益率排名 |
...
| 9 | `FundReturn` | 基金收益率(%) | number(18,9) | ✓ | 100.0% | 基金收益率(%)(FundReturn): 非货币及短期债券理财型基金取自公募基金净值历史表现(MF_NetValueP... |
| 10 | `FundReturnMean` | 同类基金收益率均值(%) | number(18,9) | ✓ | 100.0% |  |
| 11 | `FundAnnReturn` | 基金年化收益率(%) | number(18,9) | ✓ | 30.75% | 基金年化收益率(%)(FundAnnReturn): 非货币及短期债券理财型基金取自公募基金净值历史表现(MF_NetV... |
| 12 | `FundAnnReturnMean` | 同类基金年化收益率均值(%) | number(18,9) | ✓ | 30.75% |  |
| 13 | `FundReturnRank` | 同类基金收益率排名 | varchar2(100) | ✓ | 100.0% | 同类基金收益率排名(FundReturnRank)：成立以来同类基金收益率排名，样本范围取首日净值公布日早于本基金设立日... |
...
指标周期(IndexCycle)与(CT_SystemConst)表中的DM字段关联，令LB = 2149 AND DM NOT IN (990,996,997)，得到指标周期的具体描述：1-一个月，3-三个月，6-六个月，7-七天，12-一年，24-两年，36-三年，60-五年，120-十年，998-今年以来，999-成立以来。

### FundReturn (基金收益率(%))

基金收益率(%)(FundReturn): 非货币及短期债券理财型基金取自公募基金净值历史表现(MF_NetValuePerformanceHis）；货币及短期债券理财型基金取自公募基金货币型基金收益表现(MF_MMYieldPerformance)。


### FundAnnReturn (基金年化收益率(%))

基金年化收益率(%)(FundAnnReturn): 非货币及短期债券理财型基金取自公募基金净值历史表现(MF_NetValuePerformanceHis）；货币及短期债券理财型基金取自公募基金货币型基金收益表现(MF_MMYieldPerformance)。 

### FundReturnRank (同类基金收益率排名)

同类基金收益率排名(FundReturnRank)：成立以来同类基金收益率排名，样本范围取首日净值公布日早于本基金设立日期，且截止日期还在存续的全部同类基金。
