...
| 属性 | 值 |
|------|-----|
| 表名 | `MF_NotTextAnnouncement` |
| MySQL表名 | `mf_nottextannouncement` |
| 中文名 | 公募基金公告原文非文本 |
...
1.本表记录基金招募说明书、基金上市公告书、基金扩募说明书、基金契约、基金招募说明书更新摘要、基金招募说明书更新、基金契约修正、基金托管协议、临时公告等公告原文及临时公告内容。
2.历史数据：1998年3月起-至今。
3.数据来源：基金公司、证监会、交易所、四大报。
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `AnnouncementID` | 基金公告原文ID | number(19) | ✓ | 61.35% | 基金公告原文ID(AnnouncementID)：与基金公告原文(MF_Announcement)表中ID字段关联,直接... |
| 3 | `InterBulletinID` | 基金临时公告ID | number(19) | ✓ | 36.73% | 基金临时公告ID(InterBulletinID)：与基金临时公告(MF_InterimBulletin)表中ID字段关... |
| 4 | `InnerCode` | 基金内部编码 | number(10) | ✗ |  |  |
...
| 7 | `Media` | 媒体出处 | varchar2(100) | ✓ | 100.0% |  |
| 8 | `EndDate` | 截止日期 | date | ✓ | 100.0% |  |
| 9 | `Category` | 内容类别 | number(10) | ✓ | 100.0% | 内容类别(Category)与(CT_SystemConst)表中的DM字段关联，令LB = 1032，得到内容类别的具... |
| 10 | `InfoTitle` | 信息标题 | varchar2(200) | ✓ | 100.0% |  |
...
| 14 | `InfoType` | 信息类别 | number(10) | ✓ | 100.0% | 信息类别(InfoType)与(CT_SystemConst)表中的DM字段关联，令LB = 1311，得到信息类别的具... |
| 15 | `HashCode` | MD5校验码 | varchar2(100) | ✓ | 100.0% |  |
| 16 | `InsertTime` | 发布时间 | date | ✗ |  |  |
| 17 | `XGRQ` | 更新时间 | date | ✗ |  |  |
| 18 | `JSID` | JSID | number(19) | ✗ |  |  |
| 19 | `FileName` | 文件名 | varchar2(100) | ✓ | 4.3% |  |
| 20 | `SerialNumber` | 序号 | number(10) | ✓ | 0.0% |  |
| 21 | `SubTitle` | 小标题 | varchar2(200) | ✓ | 0.0% |  |

## 字段说明

### AnnouncementID (基金公告原文ID)

基金公告原文ID(AnnouncementID)：与基金公告原文(MF_Announcement)表中ID字段关联,直接获得相关公告内容。

### InterBulletinID (基金临时公告ID)

基金临时公告ID(InterBulletinID)：与基金临时公告(MF_InterimBulletin)表中ID字段关联,获得相关公告内容。
