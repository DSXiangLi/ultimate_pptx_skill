...
| 属性 | 值 |
|------|-----|
| 表名 | `MF_InterimBulletin` |
| MySQL表名 | `mf_interimbulletin` |
| 中文名 | 公募基金临时公告 |
...
1.本表记录基金历年披露的除发行上市公告书、定期报告等以外的其他各类公告；其他机构如交易所、证监会等发布的相关公告。
2.历史数据：1998年3月起-至今。
3.数据来源：基金公司官网。
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `RID` | 非文本ID | number(19) | ✓ | 96.68% |  |
| 3 | `BulletinDate` | 公告日期 | date | ✗ | 100.0% |  |
| 4 | `InfoTitle` | 信息标题 | varchar2(200) | ✗ | 100.0% |  |
| 5 | `Detail` | 详细内容 | clob | ✓ | 0.29% |  |
| 6 | `BulletinType` | 公告类型 | number(10) | ✓ | 100.0% | 公告类型(BulletinType)与(CT_SystemConst)表中的DM字段关联，令LB = 1109，得到公告... |
| 7 | `Market` | 对应市场 | number(10) | ✓ | 19.38% | 对应市场(Market)与(CT_SystemConst)表中的DM字段关联，令LB = 201，得到对应市场的具体描述... |
| 8 | `Media` | 媒体出处 | varchar2(80) | ✓ | 88.08% | 媒体出处(Media)与(CT_SystemConst)表中的DM字段关联，令LB = 1183 ，得到对应媒体的具体描... |
| 9 | `InvolvedFund` | 相关基金内部编码 | number(10) | ✓ |  |  |
| 10 | `RecordDate` | 记录录入时间 | date | ✓ | 100.0% |  |
| 11 | `XGRQ` | 更新时间 | date | ✗ |  |  |
...
媒体出处(Media)与(CT_SystemConst)表中的DM字段关联，令LB = 1183 ，得到对应媒体的具体描述：1-中国证券报；2-证券时报；3-上海证券报，等。
