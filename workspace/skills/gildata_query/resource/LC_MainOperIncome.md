...
| 属性 | 值 |
|------|-----|
| 表名 | `LC_MainOperIncome` |
| MySQL表名 | `lc_mainoperincome` |
| 中文名 | 公司主营业务构成 |
...
1收录公司主营业务的收入来源、成本构成；主营业务收入、成本和利润与上年同期的对比较。
2.数据范围：1998-12-31至今
...
| 5 | `DateType` | 日期类型 | number(10) | ✓ | 100.0% | 日期类型(DateType)与(CT_SystemConst)表中的DM字段关联，令LB = 1074，得到日期类型的具... |
...
| 9 | `IfAdjusted` | 调整标志 | number(10) | ✓ | 100.0% | 调整标志(IfAdjusted)：与“系统常量表（CT_SystemConst）”中的“代码（DM）”关联，令“LB=1... |
| 10 | `Classification` | 分类方式 | number(10) | ✓ | 100.0% | 分类方式(Classification)与(CT_SystemConst)表中的DM字段关联，令LB = 1046 AN... |
| 11 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 100.0% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币... |
| 12 | `SN` | 序号 | number(10) | ✓ | 100.0% |  |
| 13 | `Project` | 经营项目名称 | varchar2(255) | ✓ | 100.0% | 经营项目名称(Project)：经营项目名称中“其他业务”对应展示原文披露的其他业务收入/成本，与主营业务区分列示。 |
| 14 | `Level` | 所属层级 | number(10) | ✓ | 99.82% | 所属层级（Level）展示各字段对应层级：如合计--0级；合计下面的一级明细项--1级；一级明细项对应的其中项--2级，... |
| 15 | `ParentName` | 上级科目名称 | varchar2(255) | ✓ | 85.24% | 上级科目名称（ParentName）展示各字段对应上级科目名称：如“合计”对应上级为空；“合计下面的一级明细项”对应上级... |
| 16 | `MainOperIncome` | 主营业务收入(元) | number(19,4) | ✓ | 97.77% |  |
| 17 | `MainOperCost` | 主营业务成本(元) | number(19,4) | ✓ | 71.43% |  |
| 18 | `MainOperProfit` | 主营业务利润(元) | number(19,4) | ✓ | 71.88% | 主营业务利润(MainOperProfit)：优先取原文披露值，如未披露，则计算，计算公式=当主营业务收入>0时，主营业... |
| 19 | `GrossProfit` | 毛利率 | number(14,8) | ✓ | 73.53% | 毛利率(GrossProfit)：优先取原文披露值，如未披露，则计算，计算公式=当主营业务收入>0时，（主营业务收入-主... |
| 20 | `MainOperIncomeFormerYear` | 上年同期主营业务收入(元) | number(19,4) | ✓ | 40.76% |  |
| 21 | `MainOperCostFormerYear` | 上年同期主营业务成本(元) | number(19,4) | ✓ | 26.0% |  |
| 22 | `MainOperProfitFormerYear` | 上年同期主营业务利润(元) | number(19,4) | ✓ | 25.56% | 上年同期主营业务利润(元)(MainOperProfitFormerYear)：优先取原文披露值，如未披露，则计算，计算... |
| 23 | `GrossProfitFormerYear` | 上年同期毛利率 | number(14,8) | ✓ | 25.79% | 上年同期毛利率(GrossProfitFormerYear)：优先取原文披露值，如未披露，则计算，计算公式=当上年同期主... |
| 24 | `MainIncomeGrowRateYOY` | 主营业务收入同比 | number(18,4) | ✓ | 44.11% | 主营业务收入同比(MainIncomeGrowRateYOY)：优先取原文披露值，如未披露，则计算，计算公式=（主营业务... |
| 25 | `MainICostGrowRateYOY` | 主营业务成本同比 | number(18,4) | ✓ | 31.3% | 主营业务成本同比(MainICostGrowRateYOY)：优先取原文披露值，如未披露，则计算，计算公式=（主营业务成... |
| 26 | `MainProfitGrowRateYOY` | 主营业务利润同比 | number(18,4) | ✓ | 24.58% | 主营业务利润同比(MainProfitGrowRateYOY)：优先取原文披露值，如未披露，则计算，计算公式=（主营业务... |
| 27 | `GrossProfitYOY` | 毛利率同比 | number(18,4) | ✓ | 24.73% | 毛利率同比(GrossProfitYOY)=（毛利率-上年同期毛利率）/ABS上年同期毛利率。 |
| 28 | `GrossProfitIncrease` | 毛利率比上年同期增减 | number(14,8) | ✓ | 31.3% | 毛利率比上年同期增减(GrossProfitIncrease)：优先取原文披露值，如未披露，则计算，计算公式=毛利率-上... |
| 29 | `InsertTime` | 发布时间 | date | ✓ |  |  |
| 30 | `XGRQ` | 修改日期 | date | ✗ |  |  |
| 31 | `JSID` | JSID | number(19) | ✗ |  |  |
| 32 | `Industry` | 所属行业 | number(10) | ✓ | 4.41% | 所属行业(Industry)：该字段暂不维护 |
| 33 | `RegionAndBusiness` | 地区与业务性质 | number(10) | ✓ | 29.66% | 地区与业务性质(RegionAndBusiness)：该字段暂不维护。 |
...
调整标志(IfAdjusted)：与“系统常量表（CT_SystemConst）”中的“代码（DM）”关联，令“LB=1188”，得到财务报表“调整标志”的具体描述。1-是，2-否，3-前，其中3-前代表数据更正的时候，会把之前的数据的调整标志改成前，此处理2010年开始废弃。

### Classification (分类方式)

分类方式(Classification)与(CT_SystemConst)表中的DM字段关联，令LB = 1046 AND DM IN (10,20,30)，得到分类方式的具体描述：10-按行业，20-按产品，30-按地区。
...
经营项目名称(Project)：经营项目名称中“其他业务”对应展示原文披露的其他业务收入/成本，与主营业务区分列示。

### Level (所属层级)

所属层级（Level）展示各字段对应层级：如合计--0级；合计下面的一级明细项--1级；一级明细项对应的其中项--2级，依次类推。当原文披露明细同时包含两种层级关系时，会对披露维度更为精确、明细的按照对应层级标注（1、2、3、4......），另外一种则作为“其中项”不参与运算，所属层级赋值“99”。（具体样例参见：Q&A）

### ParentName (上级科目名称)

上级科目名称（ParentName）展示各字段对应上级科目名称：如“合计”对应上级为空；“合计下面的一级明细项”对应上级为“合计”；“一级明细项对应的其中项”对应上级为“一级明细项”，依次类推。
