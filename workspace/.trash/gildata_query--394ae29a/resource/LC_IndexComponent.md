...
| 属性 | 值 |
|------|-----|
| 表名 | `LC_IndexComponent` |
| MySQL表名 | `lc_indexcomponent` |
| 中文名 | 指数成份 |
...
1.收录了市场上主要指数的成份证券构成情况，包括成份证券的市场代码、入选日期、删除日期以及成份标志等信息。
...
| 4 | `SecuMarket` | 成份股市场代码 | number(10) | ✓ | 97.88% | 成份股市场代码(SecuMarket)与(CT_SystemConst)表中的DM字段关联，令LB = 201，得到成份... |
| 5 | `InDate` | 入选日期 | date | ✓ | 100.0% |  |
| 6 | `OutDate` | 剔除日期 | date | ✓ | 67.56% |  |
| 7 | `Flag` | 成份标志 | number(10) | ✗ | 100.0% | 成份标志（Flag），该字段固定常量以下常量：1-是；0-否 |
| 8 | `XGRQ` | 更新时间 | date | ✗ |  |  |
