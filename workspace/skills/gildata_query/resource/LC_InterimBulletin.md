...
| 属性 | 值 |
|------|-----|
| 表名 | `LC_InterimBulletin` |
| MySQL表名 | `lc_interimbulletin` |
| 中文名 | 公司临时公告 |
...
1.公司历年披露的除发行上市公告书、定期报告等以外的其他各类公告，其他机构如交易所、证监会、拍卖行等发布的相关公告。
2.数据范围：证券上市-至今
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `BulletinDate` | 公告日期 | date | ✗ | 100.0% |  |
| 3 | `InfoTitle` | 信息标题 | varchar2(200) | ✗ | 100.0% |  |
| 4 | `Detail` | 详细内容 | clob | ✓ | 1.92% |  |
| 5 | `BulletinType` | 公告类型 | number(10) | ✓ | 100.0% | 公告类型(BulletinType)与(CT_SystemConst)表中的DM字段关联，令LB = 1109，得到公告... |
| 6 | `Market` | 对应市场 | number(10) | ✓ | 91.93% | 对应市场(Market)与(CT_SystemConst)表中的DM字段关联，令LB = 201，得到对应市场的具体描述... |
| 7 | `Category` | 对应类别 | number(10) | ✓ | 99.92% | 对应类别(Category)与(CT_SystemConst)表中的DM字段关联，令LB = 1177，得到对应类别的具... |
| 8 | `Media` | 媒体出处 | number(10) | ✓ | 98.02% | 媒体出处(Media)与(CT_SystemConst)表中的DM字段关联，令LB = 1183，得到媒体出处的具体描述... |
| 9 | `InvolvedStock` | 相关股票 | number(10) | ✓ |  |  |
| 10 | `Keywords` | 相关关键词 | number(10) | ✓ |  |  |
| 11 | `RecordDate` | 记录录入时间 | date | ✓ | 97.17% |  |
