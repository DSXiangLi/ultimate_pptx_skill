...
| 属性 | 值 |
|------|-----|
| 表名 | `LC_IPOQuoteDetails` |
| MySQL表名 | `lc_ipoquotedetails` |
| 中文名 | A股IPO投资者报价情况 |
...
1.内容说明：收录A股新上市公司首次发行股票公告中披露的投资者报价信息
3.信息来源：上市公司首次发行股票公告
...
| 9 | `InvestorID` | 投资者编码 | number(10) | ✓ | 89.77% | 投资者编码(InvestorID):根据“InvestorType(投资者类型)”来确定, 当InvestorType(... |
...
| 13 | `BidderID` | 配售对象编码 | number(10) | ✓ | 67.71% | 配售对象编码(BidderID)：根据“BidderType(配售对象类型)”来确定, 当BidderType(配售对象... |
| 14 | `BidderCode` | 配售对象代码 | varchar2(20) | ✓ | 6.03% |  |
| 15 | `SecuAccountNumber` | 证券账户号码 | varchar2(20) | ✓ | 70.99% |  |
| 16 | `PriceUnit` | 申购价格(元/股) | number(19,4) | ✓ | 100.0% |  |
| 17 | `PlannedBidVol` | 拟申购数量(股) | number(16,0) | ✓ | 100.0% |  |
...
| 23 | `LimitSaleType` | 限售方式 | number(10) | ✓ | 0.0% | 限售方式(LimitSaleType)与(CT_SystemConst)表中的DM字段关联，令LB=2668，得到限售方... |
| 24 | `LimitSalePeriod` | 限售期 | number(10) | ✓ | 0.0% |  |
| 25 | `LimitSalePeriodUnit` | 限售期单位 | number(10) | ✓ | 0.0% | 限售期单位(LimitSalePeriodUnit)与(CT_SystemConst)表中的DM字段关联，令LB=102... |
| 26 | `RestriHoldingRatio` | 限售比例(%) | number(9,4) | ✓ | 0.0% |  |
