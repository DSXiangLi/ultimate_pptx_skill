...
1.公司税负变动等重大事项，包括事件主体/交易对象名称、企业编号、与上市公司关联关系、税负类别、税负变动对象名称、税负变动前利率、税负变动后利率、税负优惠方式、变动后税率执行起始日、执行截止日等指标。
2.数据范围：2001-至今
3.信息来源：上市公司公告
...
| 3 | `InitialInfoPublDate` | 首次信息发布日期 | date | ✓ | 97.19% |  |
| 4 | `InfoPublDate` | 信息发布日期 | date | ✗ | 100.0% |  |
| 5 | `InfoSource` | 信息来源 | varchar2(50) | ✓ | 97.19% |  |
| 6 | `AnnouncementType` | 公告类型 | number(10) | ✓ | 100.0% | 公告类型(AnnouncementType)与(CT_SystemConst)表中的DM字段关联，令LB = 1109，... |
| 7 | `DisclosureMethod` | 披露方式 | number(10) | ✓ | 97.19% | 披露方式(DisclosureMethod)与(CT_SystemConst)表中的DM字段关联，令LB = 1221，... |
| 8 | `EventContent` | 事件内容 | clob | ✓ | 2.68% |  |
| 9 | `ActionDesc` | 行为描述 | varchar2(200) | ✓ | 97.19% |  |
| 10 | `NewestAdvance` | 最新进展状态描述 | varchar2(200) | ✓ | 0.86% |  |
| 11 | `EventSubject` | 事件主体 | number(10) | ✓ | 100.0% | 事件主体(EventSubject)与(CT_SystemConst)表中的DM字段关联，令LB = 1246，得到事件... |
| 12 | `EventProcedure` | 事件进程 | number(10) | ✓ | 97.19% | 事件进程(EventProcedure)与(CT_SystemConst)表中的DM字段关联，令LB = 1059，得到... |
| 13 | `ActionWays` | 行为方式 | number(10) | ✓ | 97.19% | 行为方式(ActionWays)与(CT_SystemConst)表中的DM字段关联，令LB = 1063，得到行为方式... |
| 14 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 97.19% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币... |
| 15 | `SubjectName` | 事件主体名称 | varchar2(100) | ✓ | 97.19% |  |
| 16 | `SubjectCode` | 事件主体企业编号 | number(10) | ✓ | 5.04% | 事件主体企业编号（SubjectCode）：与“机构基本资料表（LC_InstiArchive）”中“公司代码（Comp... |
| 17 | `SubjectAssociation` | 与上市公司关联关系 | number(10) | ✓ | 97.19% | 与上市公司关联关系(SubjectAssociation)与(CT_SystemConst)表中的DM字段关联，令LB ... |
| 18 | `ObjectName` | 交易对象名称 | varchar2(100) | ✓ | 97.19% |  |
| 19 | `ObjectCode` | 交易对象企业编号 | number(10) | ✓ | 0.02% | 交易对象企业编号（ObjectCode）：与“机构基本资料表（LC_InstiArchive）”中“公司代码（Compa... |
| 20 | `ObjectAssociation` | 与上市公司关联关系 | number(10) | ✓ | 97.19% | 与上市公司关联关系(ObjectAssociation)与(CT_SystemConst)表中的DM字段关联，令LB =... |
| 21 | `AgreementDate` | 协议签署日期 | date | ✓ | 0.46% |  |
| 22 | `IfEnded` | 是否终止 | number(10) | ✓ | 0.72% | 是否终止（IfEnded），该字段固定以下常量：0-否；1-是 |
| 23 | `Note` | 备注 | varchar2(300) | ✓ | 0.01% |  |
| 24 | `TaxType` | 税负类别 | number(10) | ✓ | 97.74% | 税负类别(TaxType)与(CT_SystemConst)表中的DM字段关联，令LB = 1134，得到税负类别的具体... |
| 25 | `TaxChangeObject` | 税负变动对象名称 | varchar2(100) | ✓ | 97.19% |  |
| 26 | `ObjectType` | 税负变动对象类别 | number(10) | ✓ | 100.0% | 税负变动对象类别(ObjectType)与(CT_SystemConst)表中的DM字段关联，令LB = 1305，得到... |
| 27 | `TaxRatePreChange` | 税负变动前利率 | number(9,6) | ✓ | 95.37% |  |
| 28 | `TaxRateAfterChange` | 税负变动后利率 | number(9,6) | ✓ | 96.16% |  |
| 29 | `TaxFavorMethod` | 税负优惠方式 | number(10) | ✓ | 95.56% | 税负优惠方式(TaxFavorMethod)与(CT_SystemConst)表中的DM字段关联，令LB = 1227，... |
| 30 | `ImplementTerm` | 变动后税率执行期限(月) | number(18,4) | ✓ | 90.77% |  |
| 31 | `ImplementTermBeginDate` | 变动后税率执行起始日 | date | ✓ | 87.82% |  |
| 32 | `ImplementTermEndDate` | 变动后税率执行截止日 | date | ✓ | 85.83% |  |
| 33 | `XGRQ` | 修改日期 | date | ✗ |  |  |
...
事件主体企业编号（SubjectCode）：与“机构基本资料表（LC_InstiArchive）”中“公司代码（CompanyCode）”关联，得到事件主体的具体信息。
...
交易对象企业编号（ObjectCode）：与“机构基本资料表（LC_InstiArchive）”中“公司代码（CompanyCode）”关联，得到交易对象的具体信息。
