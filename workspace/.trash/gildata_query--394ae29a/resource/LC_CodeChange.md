...
收录证券代码的历次变更情况，包括证券市场、代码定义、启用日期和停用日期等内容。
...
| 3 | `SecuAbbr` | 证券简称 | varchar2(100) | ✓ | 2.94% | 仅当代码定义(CodeDefine)   in   “16-上交所质押式回购申报代码、1-交易所”时有数据 |
| 4 | `SecuMarket` | 证券市场 | number(10) | ✓ | 94.98% | 证券市场(SecuMarket)与(CT_SystemConst)表中的DM字段关联，令LB = 201，得到证券市场的... |
...
| 8 | `BeginDate` | 启用日期 | date | ✓ | 76.32% |  |
| 9 | `StopDate` | 停用日期 | date | ✓ | 75.21% |  |
...
仅当代码定义(CodeDefine)   in   “16-上交所质押式回购申报代码、1-交易所”时有数据
