...
|------|-----|
| 表名 | `LC_CDRTransRatio` |
| MySQL表名 | `lc_cdrtransratio` |
| 中文名 | CDR与基础证券转换 |
| 路径 | 聚源新版数据库 > 中国存托凭证数据库 >   基本资料 |
| 更新频率 | 日更新 |
| 字段数量 | 17 |
| 版本 | 1.02 |
...
|------|--------|--------|------|------|--------|------|
| 1 | `ID` | ID | number(19) | ✗ |  |  |
| 2 | `InnerCode` | 内部编码 | number(10) | ✗ | 100.0% | 内部编码（InnerCode）：与“证券主表（SecuMain）”中的“内部编码（InnerCode）”关联，得到CDR... |
| 3 | `DRSharesNum` | 存托凭证份数 | number(18,2) | ✗ | 100.0% |  |
| 4 | `RelatedInnerCode` | 基础证券内部编码 | number(10) | ✓ | 0.0% | 基础证券内部编码（RelatedInnerCode）：与“英国证券主表（UK_SecuMain）”中的“内部编码（Inn... |
| 5 | `BasicSecuSharesNum` | 存托基础证券股数 | number(18,2) | ✗ | 100.0% |  |
| 6 | `BasicSecuType` | 基础证券种类 | varchar2(50) | ✓ | 100.0% |  |
| 7 | `TransRatio` | 转换比率 | number(18,6) | ✗ | 100.0% |  |
| 8 | `DRParValue` | 存托凭证面值 | number(19,4) | ✓ | 0.0% |  |
| 9 | `BasicSecuParValue` | 基础股票面值 | number(19,4) | ✓ | 100.0% |  |
| 10 | `DRParValueUnit` | 存托凭证面值单位 | number(10) | ✓ | 0.0% | 存托凭证面值单位(DRParValueUnit)与(CT_SystemConst )表中的DM字段关联，令LB = 10... |
| 11 | `BasicSecuParValueUnit` | 基础股票面值单位 | number(10) | ✓ | 100.0% | 基础股票面值单位(BasicSecuParValueUnit)与(CT_SystemConst )表中的DM字段关联，令... |
| 12 | `DRType` | 存托凭证种类 | number(10) | ✓ | 100.0% | 存托凭证种类(DRType)与(CT_SystemConst )表中的DM字段关联，令LB = 1177 and DM ... |
| 13 | `EffectiveDate` | 生效日期 | date | ✗ | 100.0% |  |
| 14 | `ExpiryDate` | 失效日期 | date | ✓ | 0.0% |  |
| 15 | `IfEffected` | 是否有效 | number(10) | ✗ | 100.0% |  |
