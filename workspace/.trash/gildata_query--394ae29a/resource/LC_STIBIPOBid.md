...
1.内容说明：收录A股科创板新上市询价、网下配售明细，包括投资者名称、配售对象名称、申报价格、拟申购股数、总实际申购股数和总获配售股数等内容。
2.数据范围：2019年至今
3.信息来源：上交所
...
| 5 | `InvestorName` | 投资者名称 | varchar2(100) | ✗ | 100.0% |  |
| 6 | `InvestorType` | 投资者类型 | number(10) | ✓ | 100.0% | 投资者类型(InvestorType)与(CT_SystemConst)表中的DM字段关联，令LB = 1783，得到投... |
| 7 | `InvestorCode` | 投资者编码 | number(10) | ✓ | 100.0% | 投资者编码(InvestorCode):根据“InvestorType(投资者类型)”来确定, 当InvestorTyp... |
...
| 10 | `BidderID` | 配售对象编码 | number(10) | ✓ | 73.87% | 配售对象编码(BidderID)：根据“BidderType(配售对象类型)”来确定, 当BidderType(配售对象... |
...
| 14 | `BidVolEx` | 被剔除申购股数(股/份) | number(19,2) | ✓ | 77.41% |  |
| 15 | `ActualBidVol` | 总实际申购股数(股/份) | number(19,2) | ✓ | 77.41% |  |
| 16 | `ActualAllotment` | 总获配售股数(股/份) | number(19,2) | ✓ | 99.37% |  |
| 17 | `InsertTime` | 发布时间 | date | ✗ |  |  |
| 18 | `UpdateTime` | 更新时间 | date | ✗ |  |  |
| 19 | `JSID` | JSID | number(19) | ✗ |  |  |
| 20 | `BidderCode` | 配售对象代码 | number(10) | ✓ | 0.0% | 配售对象为企业时，与机构基本资料（LC_InstiArchive）中的企业编号（CompanyCode）关联；配售对象为... |
| 21 | `BidderCategory` | 配售对象类别 | number(10) | ✓ | 64.65% | 配售对象类别(BidderCategory)与(CT_SystemConst)表中的DM字段关联，令LB = 1825，... |
...
配售对象为企业时，与机构基本资料（LC_InstiArchive）中的企业编号（CompanyCode）关联；配售对象为理财产品时，与理财产品主表（SF_PlanMain）或证券主表（SecuMain）中的内部编码（InnerCode）关联。
