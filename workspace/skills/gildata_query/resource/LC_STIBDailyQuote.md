...
| 属性 | 值 |
|------|-----|
| 表名 | `LC_STIBDailyQuote` |
| MySQL表名 | `lc_stibdailyquote` |
| 中文名 | 科创板日行情 |
...
1.内容说明：收录科创板每个交易日收盘行情数据，包括昨收盘、今开盘、最高价、最低价、收盘价、成交量、成交金额、成交笔数等行情指标。
2.数据范围：证券上市起-至今
3.信息来源：上交所每日行情文件
...
| 9 | `TurnoverVolume` | 收盘成交量(股/份) | number(19,0) | ✓ | 100.0% |  |
| 10 | `TurnoverValue` | 收盘成交金额(元) | number(19,4) | ✓ | 100.0% |  |
| 11 | `TurnoverDeals` | 收盘成交笔数(笔) | number(10) | ✓ | 100.0% |  |
