...
| 属性 | 值 |
|------|-----|
| 表名 | `CS_HKStampDutyExRates` |
| MySQL表名 | `cs_hkstampdutyexrates` |
| 中文名 | 港股印花税汇率表 |
...
1.内容说明：记录香港交易所用于计算印花税等相关税费的汇率数据
2.数据范围：2023年3月至今。
3.信息来源：港交所。
...
| 3 | `DomeCurrency` | 本币种类 | number(10) | ✗ | 100.0% | 本币种类(DomeCurrency)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到本币... |
| 4 | `ForeCurrency` | 外币种类 | number(10) | ✗ | 100.0% | 外币种类(ForeCurrency)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到外币... |
| 5 | `BuyingTT` | 电汇买入价 | number(19,8) | ✓ | 100.0% |  |
