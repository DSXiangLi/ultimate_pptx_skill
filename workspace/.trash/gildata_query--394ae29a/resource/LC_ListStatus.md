...
收录各证券上市状态变更的情况，包括证券市场、变更日期、变更类型、变更说明等内容。
...
| 4 | `SecuMarket` | 证券市场 | number(10) | ✓ | 42.31% | 证券市场(SecuMarket)与(CT_SystemConst)表中的DM字段关联，令LB = 201，得到证券市场的... |
| 5 | `ChangeType` | 变更类型 | number(10) | ✓ | 100.0% | 变更类型(ChangeType)与(CT_SystemConst)表中的DM字段关联，令LB = 1184，得到变更类型... |
| 6 | `LargeApplyingMax` | 基金大额申购上限(元) | number(19,4) | ✓ | 8.98% | 基金大额申购上限（LargeApplyingMax）：“变更类型（ChangeType）”为“限制大额申购”或“恢复大额... |
| 7 | `Statement` | 变更说明 | varchar2(400) | ✓ | 1.12% |  |
| 8 | `ChangeReason` | 变动原因说明 | number(10) | ✓ | 0.0% |  |
| 9 | `InsertTime` | 发布时间 | date | ✓ |  | 指该条记录首次在本表发布的时间。 |
| 10 | `XGRQ` | 修改日期 | date | ✗ |  |  |
...
基金大额申购上限（LargeApplyingMax）：“变更类型（ChangeType）”为“限制大额申购”或“恢复大额申购”，该字段处理为大额申购上限数据。
