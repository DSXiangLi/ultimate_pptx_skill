...
| 属性 | 值 |
|------|-----|
| 表名 | `LC_SuitArbitration` |
| MySQL表名 | `lc_suitarbitration` |
| 中文名 | 公司诉讼仲裁明细 |
...
| 3 | `InitialInfoPublDate` | 首次信息发布日期 | date | ✓ | 95.43% |  |
| 4 | `InfoPublDate` | 信息发布日期 | date | ✗ | 100.0% |  |
| 5 | `InfoSource` | 信息来源 | varchar2(50) | ✓ | 95.43% |  |
| 6 | `AnnouncementType` | 公告类型 | number(10) | ✓ | 100.0% | 公告类型(AnnouncementType)与(CT_SystemConst)表中的DM字段关联，令LB = 1109，... |
| 7 | `DisclosureMethod` | 披露方式 | number(10) | ✓ | 95.43% | 披露方式(DisclosureMethod)与(CT_SystemConst)表中的DM字段关联，令LB = 1221，... |
| 8 | `EventContent` | 事件内容 | clob | ✓ | 1.31% |  |
| 9 | `ActionDesc` | 行为描述 | varchar2(200) | ✓ | 95.43% |  |
| 10 | `NewestAdvance` | 最新进展状态描述 | varchar2(200) | ✓ | 49.88% |  |
| 11 | `EventSubject` | 事件主体 | number(10) | ✓ | 99.91% | 事件主体(EventSubject)与(CT_SystemConst)表中的DM字段关联，令LB = 1246，得到事件... |
| 12 | `EventProcedure` | 事件进程 | number(10) | ✓ | 95.43% | 事件进程(EventProcedure)与(CT_SystemConst)表中的DM字段关联，令LB = 1059，得到... |
| 13 | `ActionWays` | 行为方式 | number(10) | ✓ | 95.43% | 行为方式(ActionWays)与(CT_SystemConst)表中的DM字段关联，令LB = 1063，得到行为方式... |
| 14 | `CurrencyUnit` | 货币单位 | number(10) | ✓ | 95.43% | 货币单位(CurrencyUnit)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币... |
| 15 | `SubjectName` | 事件主体名称 | varchar2(100) | ✓ | 95.43% |  |
| 16 | `SubjectCode` | 事件主体企业编号 | number(10) | ✓ | 1.8% | 事件主体企业编号(SubjectCode)和机构基本资料表(CompanyCode)关联 |
| 17 | `SubjectAssociation` | 与上市公司关联关系 | number(10) | ✓ | 95.43% | 与上市公司关联关系(SubjectAssociation)与(CT_SystemConst)表中的DM字段关联，令LB ... |
| 18 | `ObjectName` | 交易对象名称 | varchar2(100) | ✓ | 95.43% |  |
| 19 | `ObjectCode` | 交易对象企业编号 | number(10) | ✓ | 0.98% | 交易对象企业编号(ObjectCode)和机构基本资料表(CompanyCode)关联 |
| 20 | `ObjectAssociation` | 与上市公司关联关系 | number(10) | ✓ | 95.43% | 与上市公司关联关系(ObjectAssociation)与(CT_SystemConst)表中的DM字段关联，令LB =... |
| 21 | `AgreementDate` | 协议签署日期 | date | ✓ | 0.23% |  |
| 22 | `IfEnded` | 是否终止 | number(10) | ✓ | 0.64% | 是否终止（IfEnded），该字段固定以下常量：0-否；1-是 |
| 23 | `Note` | 备注 | varchar2(300) | ✓ | 0.04% |  |
| 24 | `FirstSuitSum` | 首次诉讼仲裁金额(元) | number(19,4) | ✓ | 86.0% |  |
| 25 | `LatestSuitSum` | 最新诉讼仲裁涉及金额(元) | number(19,4) | ✓ | 89.7% |  |
| 26 | `Plaintiff` | 诉讼仲裁原告 | varchar2(100) | ✓ | 99.42% |  |
| 27 | `PlaintiffAssociation` | 与上市公司关联关系 | number(10) | ✓ | 100.0% | 与上市公司关联关系(PlaintiffAssociation)与(CT_SystemConst)表中的DM字段关联，令L... |
| 28 | `Defendant` | 诉讼仲裁被告 | varchar2(100) | ✓ | 98.51% |  |
| 29 | `DefendantAssociation` | 与上市公司关联关系 | number(10) | ✓ | 100.0% | 与上市公司关联关系(DefendantAssociation)与(CT_SystemConst)表中的DM字段关联，令L... |
| 30 | `JSRParty` | 诉讼仲裁连带责任人 | varchar2(100) | ✓ | 10.49% |  |
| 31 | `JSRPartyAssociation` | 与上市公司关联关系 | number(10) | ✓ | 9.7% | 与上市公司关联关系(JSRPartyAssociation)与(CT_SystemConst)表中的DM字段关联，令LB... |
| 32 | `OtherParty` | 其他方 | varchar2(50) | ✓ | 6.08% |  |
| 33 | `OtherPartyAssociation` | 其他方与上市公司关联关系 | number(10) | ✓ | 6.08% | 其他方与上市公司关联关系(OtherPartyAssociation)与(CT_SystemConst)表中的DM字段关... |
| 34 | `SubjectMatterStat` | 案由描述 | varchar2(200) | ✓ | 95.42% |  |
| 35 | `SubjectMatter` | 案由 | number(10) | ✓ | 100.0% | 案由(SubjectMatter)与(CT_SystemConst)表中的DM字段关联，令LB = 1228，得到案由的... |
| 36 | `EventSubjectRole` | 事件主体在诉讼中的角色 | number(10) | ✓ | 100.0% | 事件主体在诉讼中的角色(EventSubjectRole)与(CT_SystemConst)表中的DM字段关联，令LB ... |
| 37 | `InquisitionInstitute` | 诉讼仲裁审理机构 | varchar2(50) | ✓ | 95.41% |  |
| 38 | `CaseStatus` | 其中：仲裁状态 | number(10) | ✓ | 7.84% | 其中：仲裁状态(CaseStatus)与(CT_SystemConst)表中的DM字段关联，令LB = 1065，得到其... |
| 39 | `FirstInstanceStatus` | 其中：诉讼一审状态 | number(10) | ✓ | 87.59% | 其中：诉讼一审状态(FirstInstanceStatus)与(CT_SystemConst)表中的DM字段关联，令LB... |
| 40 | `SecondInstanceStatus` | 其中：诉讼二审状态 | number(10) | ✓ | 17.71% | 其中：诉讼二审状态(SecondInstanceStatus)与(CT_SystemConst)表中的DM字段关联，令L... |
| 41 | `SPPStatus` | 其中：最高院监督状态 | number(10) | ✓ | 2.34% | 其中：最高院监督状态(SPPStatus)与(CT_SystemConst)表中的DM字段关联，令LB = 1061，得... |
| 42 | `PropertyEnforcement` | 财产执行情况 | number(10) | ✓ | 11.03% | 财产执行情况(PropertyEnforcement)与(CT_SystemConst)表中的DM字段关联，令LB = ... |
| 43 | `PropertyEnforced` | 被执行财产 | varchar2(100) | ✓ | 11.08% |  |
| 44 | `PropertyBelonged` | 被执行财产归属 | number(10) | ✓ |  |  |
| 45 | `AdjudgementStatus` | 判决执行状态 | number(10) | ✓ | 8.93% | 判决执行状态(AdjudgementStatus)与(CT_SystemConst)表中的DM字段关联，令LB = 10... |
| 46 | `XGRQ` | 修改日期 | date | ✗ |  |  |
