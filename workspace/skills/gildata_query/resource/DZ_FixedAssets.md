...
1.内容说明：
1.1 描述新会计准则下，上市公司固定资产及折旧的明细情况，包括固定资产的原值、累计折旧、减值准备、账面价值、净值等各项的期初、期间变化和期末数据。
1.2 对于公告原文披露的项目名称，收录在“固定资产名称（FixedAssetName）”中；“固定资产分类（FixedAssetType）”则对披露的科目进行了归类，以便于横向比较。
3.数据范围：2000-12-31至今
...
| 10 | `CurrentIncreaseOC` | 原值本期增加(元) | number(19,4) | ✓ | 91.05% |  |
| 11 | `CurrentDecreaseOC` | 原值本期减少(元) | number(19,4) | ✓ | 99.65% |  |
| 12 | `EndingOriginalCost` | 原值期末数(元) | number(19,4) | ✓ | 99.58% |  |
| 13 | `OpeningAccuDepreciation` | 累计折旧期初数(元) | number(19,4) | ✓ | 98.44% |  |
| 14 | `CurrentIncreaseAD` | 累计折旧本期增加(元) | number(19,4) | ✓ | 98.9% |  |
| 15 | `CurrentDecreaseAD` | 累计折旧本期减少(元) | number(19,4) | ✓ | 99.27% |  |
| 16 | `EndingAccuDepreciation` | 累计折旧期末数(元) | number(19,4) | ✓ | 99.23% |  |
| 17 | `OpeningDepreReserves` | 减值准备期初数(元) | number(19,4) | ✓ | 19.22% |  |
| 18 | `CurrentIncreaseDR` | 减值准备本期增加(元) | number(19,4) | ✓ | 10.38% |  |
| 19 | `CurrentDecreaseDR` | 减值准备本期减少(元) | number(19,4) | ✓ | 11.97% |  |
| 20 | `EndingDepreReserves` | 减值准备期末数(元) | number(19,4) | ✓ | 19.72% |  |
| 21 | `OpeningBookValue` | 账面价值期初数(元) | number(19,4) | ✓ | 98.83% |  |
| 22 | `CurrentIncreaseBV` | 账面价值本期增加(元) | number(19,4) | ✓ | 48.37% |  |
| 23 | `CurrentDecreaseBV` | 账面价值本期减少(元) | number(19,4) | ✓ | 23.59% |  |
| 24 | `EndingBookValue` | 账面价值期末数(元) | number(19,4) | ✓ | 99.46% |  |
| 25 | `OpeningNetValue` | 净值期初数(元) | number(19,4) | ✓ | 98.88% |  |
| 26 | `CurrentIncreaseNV` | 净值本期增加(元) | number(19,4) | ✓ | 48.09% |  |
| 27 | `CurrentDecreaseNV` | 净值本期减少(元) | number(19,4) | ✓ | 23.3% |  |
| 28 | `EndingNetValue` | 净值期末数(元) | number(19,4) | ✓ | 99.59% |  |
