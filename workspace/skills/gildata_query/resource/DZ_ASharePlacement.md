...
| 属性 | 值 |
|------|-----|
| 表名 | `DZ_ASharePlacement` |
| MySQL表名 | `dz_ashareplacement` |
| 中文名 | A股配股 |
...
| 7 | `StockType` | 发行股票类型 | number(10) | ✓ | 100.0% | 发行股票类型(StockType)与(CT_SystemConst)表中的DM字段关联，令LB = 1177，得到发行股... |
...
| 28 | `PlannedBaseShares` | 计划配股股本基数(股) | number(16,0) | ✓ | 97.45% |  |
| 29 | `BaseShares` | 实际配股股本基数(股) | number(16,0) | ✓ | 69.83% |  |
...
| 39 | `PlaBaseDate` | 配股基准日 | date | ✓ | 46.83% |  |
| 40 | `PlaPrice` | 每股实际配股价格(元) | number(19,4) | ✓ | 69.35% |  |
