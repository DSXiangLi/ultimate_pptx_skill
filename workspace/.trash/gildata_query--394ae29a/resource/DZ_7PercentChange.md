...
| 属性 | 值 |
|------|-----|
| 表名 | `DZ_7PercentChange` |
| MySQL表名 | `dz_7percentchange` |
| 中文名 | 交易所日公开信息 |
...
1.内容说明：收录交易所公布的，触发日涨跌幅偏离值达到7％、日价格振幅达到15%、日换手率达到20％、连续三个交易日内收盘价格涨幅偏离值累计达到20%等各类披露条件的个股成交信息，包括个股总成交量、成交金额以及成交金额在前五名的营业部的成交明细等。
2.数据范围：1997-02-26至今
3.信息来源：上海证券交易所、深圳证券交易所、北京证券交易所
注意：本表目前未设置业务唯一性，请勿参考表单所示的'业务唯一性'一栏，业务主键可按照InnerCode,TradingDay,SerialNum,StatType及从表Code设置
...
| 8 | `SerialNum` | 序号 | number(10) | ✗ | 100.0% |  |
| 9 | `SalesDepartmentName` | 营业部名称 | varchar2(200) | ✗ | 100.0% |  |
| 10 | `BOCode` | 营业部编号 | number(10) | ✓ | 99.81% | 营业部编号(BOCode)与“机构基本资料(LC_InstiArchive)”中的企业编号(CompanyCode)关联... |
| 11 | `SecuCoBelongedCode` | 营业部所属券商编号 | number(10) | ✓ | 99.91% | 营业部所属券商编号(SecuCoBelongedCode)与“机构基本资料(LC_InstiArchive)”中的企业编... |
| 12 | `SecuCoBelonged` | 营业部所属券商 | varchar2(80) | ✓ | 99.91% |  |
| 13 | `TurnoverVolume` | 成交量(股) | number(16,0) | ✓ | 99.99% |  |
| 14 | `TurnoverValue` | 成交金额(元) | number(19,4) | ✓ | 99.99% |  |
| 15 | `ChangePCT` | 涨跌幅 | number(9,6) | ✓ | 8.43% |  |
| 16 | `ChangePCTOffset` | 涨跌幅偏离值 | number(9,6) | ✓ | 60.73% |  |
| 17 | `FluctuatingAmptitude` | 振幅 | number(9,6) | ✓ | 8.83% |  |
| 18 | `TurnoverRate` | 换手率 | number(9,6) | ✓ | 23.68% |  |
| 19 | `TurnoverProp` | 成交占比(%) | number(7,4) | ✓ | 0.34% |  |
| 20 | `TotalBuySaleSum` | 买卖合计金额(元) | number(19,4) | ✓ | 99.86% |  |
| 21 | `BuySum` | #买入金额(元) | number(19,4) | ✓ | 80.65% |  |
| 22 | `SaleSum` | #卖出金额(元) | number(19,4) | ✓ | 80.76% |  |
| 23 | `StatType` | 统计方式 | number(10) | ✓ | 100.0% | 统计方式(StatType)与(CT_SystemConst)表中的DM字段关联，令LB = 1363，得到统计方式的具... |
| 24 | `InvestorType` | 投资者类型 | number(10) | ✓ | 0.14% | 投资者类型(InvestorType)与(CT_SystemConst)表中的DM字段关联，令LB=2127 AND D... |
| 25 | `AccuBuySum` | 累计买入金额(元) | number(19,4) | ✓ | 0.14% |  |
| 26 | `AccuSaleSum` | 累计卖出金额(元) | number(19,4) | ✓ | 0.14% |  |
| 27 | `BuyingRatio` | 买入占比(%) | number(7,4) | ✓ | 0.14% |  |
| 28 | `SellingRatio` | 卖出占比(%) | number(7,4) | ✓ | 0.14% |  |
