...
| 属性 | 值 |
|------|-----|
| 表名 | `CS_SupplyChainRela` |
| MySQL表名 | `cs_supplychainrela` |
| 中文名 | 上市公司供应链关系表 |
...
1.内容说明：收录公司的供应商、客户、潜在供应商、潜在客户等供应链关系信息。
2.数据范围：境内上市公司
3.信息来源：招投标公告、公开市场招股说明书、年度报告、半年度报告等
...
| 3 | `RelationType` | 关系类型 | number(10) | ✗ | 100.0% | 关系类型(RelationType)，该字段固定以下常量：410-客户，420-供应商，430-潜在客户，440-潜在供... |
| 4 | `RelationName` | 关系名称 | varchar2(50) | ✓ | 100.0% |  |
| 5 | `RelatedPartyCode` | 关联方代码 | number(10) | ✗ | 100.0% | 关联方代码（RelatedPartyCode）：与“企业基本信息（EP_CompanyInfo）”的“企业编号（Comp... |
| 6 | `RelatedPartyName` | 关联方名称 | nvarchar2(200) | ✓ | 99.93% |  |
| 7 | `InfoPublDate` | 信息发布日期 | date | ✗ | 100.0% |  |
| 8 | `TradingSerialNumber` | 交易序号 | number(10) | ✗ | 100.0% | 交易序号（TradingSerialNumber）：指同一交易双方在同一信息发布日期下的多笔交易，用序号进行区分。 |
| 9 | `ReportDate` | 报告期 | date | ✓ | 33.51% | 报告期（ReportDate）：来源于招股说明书、年度报告、半年度报告数据，存放报告期截止日期。 |
| 10 | `TradingValue` | 交易金额 | number(19,4) | ✓ | 45.51% |  |
| 11 | `CurrencyCode` | 币种 | number(10) | ✓ | 100.0% | 币种(CurrencyCode)与(CT_SystemConst)表中的DM字段关联，令LB=1068，得到币种的具体描... |
| 12 | `InfoSource` | 信息来源 | varchar2(100) | ✓ | 100.0% | 信息来源(InfoSource)：指交易内容来源，目前包括招投标、主要供应商与客户关联交易、财务附注-往来款项、公司重大... |
