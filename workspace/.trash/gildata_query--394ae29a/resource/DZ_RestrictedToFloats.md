...
| 属性 | 值 |
|------|-----|
| 表名 | `DZ_RestrictedToFloats` |
| MySQL表名 | `dz_restrictedtofloats` |
| 中文名 | 限售股票解禁明细 |
...
内容说明：收录上市公司（包括科创板）股东历次限售股票解禁明细，包括流通起始日、股东名称、股东持股总数、新增可售A股、新增可售B股、股本变动原因等指标。
数据范围：国内上市公司
信息来源：上市公司公告
...
| 6 | `RestrictedDate` | 限售起始日期 | date | ✓ | 69.57% |  |
...
| 9 | `LimitSalePeriod` | 限售期(月) | number(19,4) | ✓ | 69.53% |  |
...
| 13 | `SHAttribute` | 股东属性 | number(10) | ✓ | 98.55% | 股东属性(SHAttribute)与(CT_SystemConst)表中的DM字段关联，令LB = 1783 and D... |
| 14 | `CompanyNumber` | 企业编号 | number(10) | ✓ | 68.99% | 当股东属性(SHAttribute)=2时，与“企业码表(EP_CompanyMain)”中的“企业编号(Company... |
| 15 | `Afloats` | 流通A股(股) | number(18,2) | ✓ | 100.0% |  |
| 16 | `AFloatListed` | 无限售流通A股(股) | number(18,2) | ✓ | 100.0% |  |
| 17 | `ManagementShares` | 高管股(股) | number(18,2) | ✓ | 0.0% |  |
| 18 | `RestrictedAFloatShares` | 有限售流通A股(股) | number(18,2) | ✓ | 87.43% |  |
...
| 29 | `ActualFloatListedSH` | 实际上市流通数量(股) | number(18,2) | ✓ | 38.06% |  |
| 30 | `ActionWays` | 解禁行为 | number(10) | ✓ | 62.08% | 解禁行为(ActionWays)与(CT_SystemConst)表中的DM字段关联，令LB = 2371，得到解禁行为... |
...
| 39 | `RestrictedCondition` | 限售条件说明 | clob | ✓ | 89.99% |  |
...
解禁行为(ActionWays)与(CT_SystemConst)表中的DM字段关联，令LB = 2371，得到解禁行为的具体描述：10-到期解禁，20-被动解禁，21-业绩补偿(被动解禁)，22-司法拍卖(被动解禁)，23-被动解禁(其他)。
