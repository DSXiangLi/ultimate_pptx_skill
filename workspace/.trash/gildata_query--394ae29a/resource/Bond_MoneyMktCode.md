...
| 属性 | 值 |
|------|-----|
| 表名 | `Bond_MoneyMktCode` |
| MySQL表名 | `bond_moneymktcode` |
| 中文名 | 货币市场代码表 |
...
1.内容说明：本表记录银行间和交易所货币市场证券编码和基本信息
2.数据范围：1995年至今
3.信息来源：上交所、深交所、货币网等
...
| 3 | `SecuCode` | 证券代码 | varchar2(10) | ✓ | 100.0% |  |
| 4 | `SecuAbbr` | 证券简称 | varchar2(100) | ✓ | 100.0% |  |
| 5 | `ChiSpelling` | 拼音证券简称 | varchar2(50) | ✓ | 100.0% |  |
| 6 | `ExtendedAbbr` | 扩位简称 | varchar2(100) | ✓ | 0.0% |  |
| 7 | `ExtendedSpelling` | 拼音扩位简称 | varchar2(50) | ✓ | 0.0% |  |
| 8 | `ChiName` | 证券全称 | varchar2(200) | ✓ | 100.0% |  |
| 9 | `ChiNameAbbr` | 中文名称缩写 | varchar2(100) | ✓ | 0.0% |  |
| 10 | `EngName` | 英文名称 | varchar2(200) | ✓ | 0.0% |  |
| 11 | `EngNameAbbr` | 英文名称缩写 | varchar2(50) | ✓ | 0.0% |  |
| 12 | `Maturity` | 期限 | number(10) | ✓ | 69.29% | 期限(Maturity)与(CT_SystemConst)表中的DM字段关联，令LB = 1157，得到期限的具体描述：... |
| 13 | `SecuMarket` | 证券市场 | number(10) | ✓ | 100.0% | 证券市场(SecuMarket)与(CT_SystemConst)表中的DM字段关联，令LB = 201，得到证券市场的... |
| 14 | `SecuCategory` | 证券类别 | number(10) | ✓ | 100.0% | 证券类别(SecuCategory)与(CT_SystemConst)表中的DM字段关联，令LB = 1177 and ... |
| 15 | `TradeType` | 交易方式 | number(10) | ✓ | 2.38% | 交易方式(TradeType)与(CT_SystemConst)表中的DM字段关联，令LB = 1556，得到交易方式的... |
| 16 | `ListedDate` | 上市日期 | date | ✓ | 41.43% |  |
| 17 | `ListedSector` | 上市板块 | number(10) | ✓ | 100.0% | 上市板块(ListedSector)与(CT_SystemConst)表中的DM字段关联，令LB = 207，得到上市板... |
| 18 | `ListedState` | 上市状态 | number(10) | ✓ | 100.0% | 上市状态(ListedState)与(CT_SystemConst)表中的DM字段关联，令LB = 1176 AND D... |
| 19 | `Currency` | 计量货币 | number(10) | ✗ | 100.0% | 计量货币(Currency)与(CT_SystemConst)表中的DM字段关联，令LB = 1068 AND DM N... |
| 20 | `InterestFormula` | 利息计算公式 | number(10) | ✓ | 17.14% | 利息计算公式(InterestFormula)与(CT_SystemConst)表中的DM字段关联，令LB = 1470... |
| 21 | `OpenTime` | 开盘时间 | varchar2(8) | ✓ | 17.26% |  |
| 22 | `CloseTime` | 收盘时间 | varchar2(8) | ✓ | 17.38% |  |
| 23 | `SettlementMode` | 交收方式 | number(10) | ✓ | 6.67% | 交收方式(SettlementMode)与(CT_SystemConst)表中的DM字段关联，令LB = 2204，得到... |
| 24 | `ISIN` | ISIN代码 | varchar2(100) | ✓ | 11.67% |  |
| 25 | `DelistDate` | 摘牌日 | date | ✓ | 30.71% |  |
...
证券类别(SecuCategory)与(CT_SystemConst)表中的DM字段关联，令LB = 1177 and DM IN ( 5,12,19,27,30,36,37,38,42,43,44,45,46)，得到证券类别的具体描述：5-国债回购，12-企业债券回购，19-买断式回购，27-新质押式回购，30-拆借，36-债券借贷，37-存款类机构质押式回购，38-存款类机构信用拆借，42-协议回购，43-三方回购，44-利率互换品种，45-标准利率互换合约，46-报价回购。

### TradeType (交易方式)

交易方式(TradeType)与(CT_SystemConst)表中的DM字段关联，令LB = 1556，得到交易方式的具体描述：1-询价，2-点击成交，3-全部，4-竞价，5-询价、点击成交。
...
交收方式(SettlementMode)与(CT_SystemConst)表中的DM字段关联，令LB = 2204，得到交收方式的具体描述：1-担保交收，2-非担保交收。
