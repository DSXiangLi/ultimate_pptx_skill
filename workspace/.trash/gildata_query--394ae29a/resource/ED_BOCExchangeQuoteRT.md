...
| 属性 | 值 |
|------|-----|
| 表名 | `ED_BOCExchangeQuoteRT` |
| MySQL表名 | `ed_bocexchangequotert` |
| 中文名 | 中国银行外汇实时牌价 |
| 路径 | 聚源新版数据库 > 现货数据库 > 外汇 |
| 更新频率 | 日更新 |
| 字段数量 | 14 |
...
1.收录中国银行发布的外汇实时价格，包括现汇买入价、现钞买入价、现汇卖出价、现钞卖出价、基准价、中行折算价
2.数据范围：2009年6月-至今
3.数据源：中国银行
...
| 4 | `InfoPublTime` | 发布时间 | date | ✗ | 100.0% |  |
| 5 | `CurrencyCode` | 货币 | number(10) | ✗ | 100.0% | 货币(CurrencyCode)与(CT_SystemConst)表中的DM字段关联，令LB = 1068，得到货币的具... |
| 6 | `BidPriceSpot` | 现汇买入价(人民币元/100外币) | number(18,6) | ✓ | 80.8% |  |
| 7 | `BidPriceCurrency` | 现钞买入价(人民币元/100外币) | number(18,6) | ✓ | 95.61% |  |
| 8 | `AskedPriceSpot` | 现汇卖出价(人民币元/100外币) | number(18,6) | ✓ | 80.97% |  |
| 9 | `AskedPriceCurrency` | 现钞卖出价(人民币元/100外币) | number(18,6) | ✓ | 92.91% |  |
| 10 | `BenchMarkPrice` | 基准价(人民币元/100外币) | number(18,6) | ✓ | 96.56% |  |
| 11 | `BOCConversionRate` | 中行折算价 | number(18,6) | ✓ | 100.0% |  |
| 12 | `Remark` | 备注说明 | varchar2(500) | ✓ | 0.0% |  |
